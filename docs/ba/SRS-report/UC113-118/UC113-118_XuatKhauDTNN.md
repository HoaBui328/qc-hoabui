# UC113-118: Báo cáo tình hình xuất khẩu của tổ chức kinh tế có vốn đầu tư nước ngoài năm ...... (Mẫu A.IV.8b)

| Thuộc tính | Giá trị |
| --- | --- |
| **BA phụ trách** | yen.le2 |
| **Phân hệ** | Quản lý đầu tư nước ngoài vào Việt Nam |
| **Mẫu biểu** | Mẫu A.IV.8b |
...
| **Ngày tạo** | 2026-04-24 |
| **Phiên bản** | 1.7 |
| **Quy tắc sinh mã báo cáo** | FDI_AIV8B_[ID] |
| **Loại quy trình** | Quy trình 3 bước, CMCĐT_BCTK_09 |

> **Lưu ý kiến trúc:** Mẫu A.IV.8b là báo cáo chi tiết **xuất khẩu** theo từng doanh nghiệp FDI. Dữ liệu từ mẫu này là **nguồn tổng hợp xuất khẩu** cho Mẫu A.IV.8a (UC107-112) — hệ thống SUM giá trị xuất khẩu theo tỉnh để điền vào cột Xuất khẩu của Mẫu 8a. Mẫu 8b (xuất khẩu) và Mẫu 8c — UC119-124 (nhập khẩu) có cấu trúc đồng nhất, chỉ khác chiều luồng hàng hóa.

---

## UC113-118.1: Xem Danh Sách Báo Cáo

### 1. Mô tả chức năng

Mô tả:

- Tên chức năng: Xem danh sách báo cáo tình hình xuất khẩu của TCKT có vốn ĐTNN (Mẫu A.IV.8b)
- Chức năng cho phép Cục Hải quan / Bộ Tài chính truy cập danh sách báo cáo định kỳ năm, được nhóm theo từng Kỳ hạn báo cáo. Mỗi đơn vị chỉ thấy báo cáo do đơn vị mình lập. Mục tiêu: Hỗ trợ nộp báo cáo xuất khẩu trực tuyến, làm nguồn dữ liệu tổng hợp cho Mẫu 8a.
...
Truy cập chức năng: Phân hệ Báo cáo → Quản lý ĐTNN vào VN → Báo cáo xuất khẩu TCKT có vốn ĐTNN (Mẫu A.IV.8b)

Chức năng đáp ứng usecase số: 113, 114, 115, 116, 117, 118
...
| 7 | Lập báo cáo | Button | Null | | | Hiển thị trong header mỗi kỳ. Chỉ hiển thị khi Trong thời hạn. Ẩn khi Chưa tới hạn hoặc đã Qua kỳ báo cáo. Tham chiếu: CF_01, CMR_04, CMR_18. |
| 8 | Nhập từ file | Button | Null | | | Hiển thị trong header mỗi kỳ. Chỉ hiển thị khi Trong thời hạn. Ẩn khi Chưa tới hạn hoặc đã Qua kỳ báo cáo. Tham chiếu: CF_02, CMR_04. |
...
| 12 | Hành động | Button group | Null | | | Chi tiết tham chiếu: UC113-118.3. |
...
## UC113-118.2: Lập Báo Cáo

### 1. Mô tả chức năng

Mô tả:

- Tên chức năng: Lập mới báo cáo tình hình xuất khẩu của TCKT có vốn ĐTNN (Mẫu A.IV.8b)
- Chức năng cho phép Cục Hải quan / Bộ Tài chính nhập liệu tình hình xuất khẩu theo từng doanh nghiệp FDI. Bảng dữ liệu hỗ trợ 2 chế độ:
...
Phân quyền: Cục Hải quan / Bộ Tài chính.

Truy cập chức năng: Màn danh sách báo cáo (UC113-118.1) → Nhấn nút [Lập báo cáo]

Chức năng đáp ứng usecase số: 113, 114
...
| **BẢNG DỮ LIỆU — DANH SÁCH DOANH NGHIỆP XUẤT KHẨU (Dynamic Table — max 500 dòng)** | | | | | | |
...
| C2 | Mã số thuế của doanh nghiệp | Text | Null | x | x | User nhập MST. Validate định dạng: **10 hoặc 13 chữ số**. Validate trùng: nếu có → lỗi inline "Mã số thuế này đã được nhập ở dòng [STT]". MST hợp lệ → gọi API CSDL Đăng ký KD auto-fill C3, C4, C5. **Mode B:** Auto-fill, vẫn cho phép chỉnh sửa. Tham chiếu: CMR_05, CMR_06. |
...
| C6 | Xuất khẩu kỳ báo cáo (USD) | Decimal | Null | x | x | **Decimal precision: tối đa 5 chữ số thập phân.** Validate ≥ 0; cấm số âm. **Mode B:** Auto-fill từ SUM tờ khai, vẫn cho phép chỉnh sửa. Tham chiếu: CMR_05. |
| **DÒNG TỔNG CỘNG** | | | | | | |
| T1 | Tổng cộng — Xuất khẩu (USD) | Tính toán | Auto | | | Read-only. SUM(C6). **Decimal: tối đa 5 chữ số thập phân.** Cập nhật real-time. |
| **FOOTER** | | | | | | |
...
| B1 | Thêm dòng | Button | | | | Thêm dòng manual vào cuối bảng. STT tự tăng. Max 500 dòng; vượt → Toast T16 *"Vượt quá số dòng tối đa cho phép (500)"*. |
...
| B3 | Lưu nháp | Button | | | | Lưu toàn bộ dữ liệu ở trạng thái Lưu nháp. Tham chiếu: CF_01 (mục "Xử lý nút [Lưu nháp]"), CMR_18. |
| B4 | Nộp báo cáo | Button | | | | Validate toàn bộ trường bắt buộc. **Empty table:** 0 dòng → Toast T15 *"Vui lòng nhập ít nhất 1 dòng dữ liệu"*, chặn nộp. Tham chiếu: CF_01, CMR_18. |
...
- Nếu MST hợp lệ → hệ thống gọi API CSDL Đăng ký KD: tìm thấy → auto-fill C3, C4, C5, vẫn cho phép chỉnh sửa; không tìm thấy → C3, C4, C5 Enabled cho nhập tay. Tham chiếu: CMR_12.
- Người dùng nhập giá trị xuất khẩu vào C6 (tối đa 5 chữ số thập phân, ≥ 0).
- Dòng Tổng cộng cập nhật real-time theo SUM(C6).

C. Mode B — API Tổng cục Hải quan:
- Hệ thống quét toàn bộ tờ khai xuất khẩu đã thông quan trong kỳ báo cáo.
- Logic lọc: Chỉ lấy tờ khai có MST thuộc danh sách doanh nghiệp FDI.
- Tính toán: SUM giá trị xuất khẩu theo từng MST → điền tự động C2, C3, C4, C5, C6.
...
E. Validate khi Lưu nháp:
- Chỉ validate bảng không trống: Nếu bảng có 0 dòng → Toast T07 *"Bạn cần nhập dữ liệu cho trường thông tin trước khi lưu nháp"*, chặn lưu.
- Không validate trường bắt buộc, không validate MST trùng khi Lưu nháp.
- Tham chiếu: CF_01 (Lưu nháp Case 2).
...
G. Liên kết với Mẫu 8a (UC107-112):
- Sau khi bản ghi được lưu nháp hoặc đã nộp, dữ liệu C5 (Tỉnh/TP) và C6 (Xuất khẩu) sẵn sàng để Mẫu 8a đọc và tổng hợp.
- **Cơ chế:** Manual — user Mẫu 8a nhấn [Tổng hợp] → query 8b đã Nộp, Group By tỉnh, SUM C6. **Không auto real-time.** 8b sửa sau → 8a KHÔNG tự cập nhật.
- **Snapshot:** Khi 8a nhấn [Tổng hợp], hệ thống lưu lại timestamp tổng hợp và mã báo cáo 8b đã sử dụng, hiển thị trên form 8a để user biết data lấy từ thời điểm nào.
...
- Cột C3 (Ngày cấp) và C6 (Xuất khẩu kỳ báo cáo) có **icon sort** trên header.
- Click icon → toggle: Ascending ↑ / Descending ↓.
  - Cột ngày (C3): xa nhất → gần nhất / gần nhất → xa nhất.
  - Cột số (C6): max → min / min → max.
- **Mặc định sort:** Theo C6 (Xuất khẩu) Descending (max → min).
...
Truy cập chức năng: Màn danh sách báo cáo (UC113-118.1) → Cột Hành động

Chức năng đáp ứng usecase số: 115, 116, 117, 118
...
- **AC1 Validate MST:** Người dùng nhập MST, nếu sai định dạng (không phải 10 hoặc 13 số) → Báo lỗi inline. Nếu nhập MST đã tồn tại trong bảng → Báo lỗi inline: "Mã số thuế này đã được nhập ở dòng [STT]". Validate on-blur + debounce 500ms.
- **AC2 Auto-fill CSDL:** Nhập MST hợp lệ và không trùng → Hệ thống tự động gọi API CSDL Đăng ký KD và điền thông tin vào cột Ngày cấp, Tên doanh nghiệp, Tỉnh/Thành phố. Các cột vẫn cho phép chỉnh sửa (Editable).
...
- **AC5 API Hải quan (Mode B):** Hệ thống quét tất cả tờ khai xuất khẩu trong kỳ theo danh sách MST FDI, tự động SUM tiền xuất khẩu theo từng MST và đổ vào bảng. Dữ liệu auto-fill vẫn cho phép user chỉnh sửa. User vẫn được thêm dòng manual bổ sung.
...
- **AC7 Tính tổng:** Cột "Tổng cộng — Xuất khẩu (USD)" luôn cập nhật real-time bằng SUM(C6) toàn bộ dòng, mỗi khi thêm/xóa/sửa dòng. Decimal: tối đa 5 chữ số thập phân.
...
- **AC11 Decimal precision:** Cột C6 (Xuất khẩu USD): tối đa 5 chữ số thập phân. Validate ≥ 0.
- **AC12 Sort:** Cột C3 (Ngày cấp) và C6 (Xuất khẩu) có icon sort trên header. Click → toggle Ascending ↑ / Descending ↓. Mặc định sort theo C6 Descending (max → min). Dòng Tổng cộng luôn ở cuối, không tham gia sort.
...
| 2026-05-07 | 2.0 | UC113-118.2 Mô tả | (Mô tả chung) | Bổ sung Mode A/B (auto-detect API Hải quan), cơ chế chuyển đổi, debounce 500ms | QC Audit — làm rõ Mode A/B. |
...
| 2026-05-07 | 2.0 | Xử lý đặc thù | (Chung) | Viết lại hoàn toàn: A-G (Mode detect, Mode A, Mode B, Partial failure, Validate, 8b→8a link, MST cross-report) | QC Audit — chi tiết hóa logic. |
...
| 2026-05-11 | 1.0→1.1 | Quy tắc sinh mã báo cáo | `DTNN_A4_8B_[ID]` | `FDI_AIV8B_[ID]` | Chuẩn hóa prefix và mã theo appendices.md v2.0 (INS-10) |
...
| 2026-05-14 | 1.4 | Sort | Không có | Icon sort C3 (Ngày cấp), C6 (Xuất khẩu); mặc định C6 Descending (max → min) | Client feedback. |
...
| 2026-05-21 | 1.5 → 1.6 | B2 Xóa dòng | Hiển thị tất cả dòng, popup [Đồng ý]/[Hủy], tham chiếu CMR_06 | Chỉ hiển thị khi ≥ 2 dòng, xóa ngay không popup. Tham chiếu CMR_15. | QC Audit — đồng bộ CMR_15 |
| 2026-05-21 | 1.5 → 1.6 | Tham chiếu CMR_02 (4 vị trí) | Tham chiếu CMR_02 | Bỏ tham chiếu CMR_02 — không áp dụng cho báo cáo Bộ/Ngành | QC Audit — fix tham chiếu sai |
| 2026-05-21 | 1.5 → 1.6 | Mode A: C3/C4/C5 + AC2 | "auto-fill và Disabled" / "Các cột này bị Disabled" | "auto-fill, vẫn cho phép chỉnh sửa" / "Editable" — đồng bộ client feedback v1.4 | QC Audit — fix mâu thuẫn nội bộ |
| 2026-05-21 | 1.5 → 1.6 | Filter Dropdown mặc định (Dòng 2, 3) | Giá trị mặc định: Null | Giá trị mặc định: Tất cả. Tham chiếu CMR_16. | QC Audit — đồng bộ CMR_16 |
| 2026-05-21 | 1.5 → 1.6 | Validate Lưu nháp | (Không có) | Bổ sung mục E: bảng trống → Toast T07, chặn lưu. Tham chiếu CF_01 Case 2. | QC Audit — bổ sung gap |
| 2026-05-21 | 1.5 → 1.6 | Hiển thị lỗi eForm Grid | (Không có) | Bổ sung: viền đỏ + tooltip. Tham chiếu CMR_05/06/07 (eForm Grid). | QC Audit — đồng bộ CMR v2.4 |
| 2026-05-21 | 1.5 → 1.6 | Tham chiếu CMR_04, CMR_18 (nút Lập/Nhập/Lưu/Nộp/Hủy) | Thiếu tham chiếu | Bổ sung CMR_04, CMR_14, CMR_18 vào các nút tương ứng | QC Audit — bổ sung tham chiếu |
| 2026-05-21 | 1.5 → 1.6 | Button ID trùng (B1) | 2 button cùng ID B1 (Thêm dòng + Hủy) | Đánh lại: B1 Thêm dòng, B5 Hủy | QC Audit — fix numbering |
| 2026-05-22 | 1.6 → 1.7 | Phân quyền Owner | Không rõ multi-user cùng cơ quan | Chỉ Owner (người tạo) được sửa/nộp/xóa; user khác chỉ Xem | QC Audit Q1 — làm rõ phân quyền |
| 2026-05-22 | 1.6 → 1.7 | Hậu điều kiện — Qua kỳ | Không đề cập hành vi Lưu nháp khi Qua kỳ | Báo cáo Lưu nháp khi kỳ Qua hạn → khóa, chỉ Xem và Xóa | QC Audit Q2 |
| 2026-05-22 | 1.6 → 1.7 | Link 8b→8a Snapshot | Không lưu thông tin snapshot | Lưu timestamp + mã báo cáo 8b đã dùng khi 8a tổng hợp | QC Audit Q4 |
| 2026-05-22 | 1.6 → 1.7 | AC | AC1-AC14 | Bổ sung AC15 (Nộp thành công → Chờ duyệt + Notification + Audit log) | QC Audit Q5 |
