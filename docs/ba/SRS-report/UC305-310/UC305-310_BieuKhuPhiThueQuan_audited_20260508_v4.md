...
**Phiên bản tài liệu:** 4.0 (Align CMR)
...
- Hệ thống đã sẵn sàng 100% để thiết kế kịch bản test. Tất cả các luồng Happy Path, Boundary Value Analysis (phần nguyên max 15 chữ số, phần thập phân max 5 chữ số — tổng 21 ký tự), Exception Path (API rỗng -> nhập tay), và các luồng thêm/xóa hàng động trên lưới (theo CMR_15) đều có thể được cover.
...
## Changelog
- **2026-05-22 (V4 — Align CMR — CRITICAL):** Sửa lỗi nghiêm trọng Numeric precision: "2 chữ số thập phân" → "Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự)". Bổ sung tham chiếu CMR_18 (Tab Navigation). Buttons ghi "Luôn Enabled". Tài liệu duy trì trạng thái READY.
