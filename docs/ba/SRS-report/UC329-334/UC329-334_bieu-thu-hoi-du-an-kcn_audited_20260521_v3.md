...
| 26 | Lập BC > Bảng > Cột 9 | Văn bản thu hồi chủ trương ĐT/GCNĐKĐT | Textbox | Yes | Null | "Nhập Văn bản thu hồi" | N/A | Nhập tay. Validate bắt buộc khi Lưu nháp/Nộp. Tham chiếu: CMR_06 | UC 329-334_ManHinhLapBaoCao.png |
...
| Các cột dữ liệu API (Cột 3, 4, 5, 6, 7, 8) | Yes (Chỉ validate khi API lỗi/null phải nhập tay) | - Trường text (Cột 3, 4, 5, 6): Trim khoảng trắng. Max length 500 ký tự.<br>- Trường số (Cột 7, 8): Số dương ≥0. Phần nguyên tối đa 15 chữ số, phần thập phân tối đa 5 chữ số (tổng 21 ký tự). Dấu phẩy phân tách hàng nghìn đúng định dạng. | — / 500 ký tự (Text)<br>— / 21 ký tự (Số: 15 nguyên + 5 thập phân) | - Trống: `"Vui lòng nhập [tên trường]"` (V01)<br>- Số âm: `"Ký tự không hợp lệ. Chỉ chấp nhận chữ số, dấu chấm và dấu phẩy"` (V03)<br>- Vượt digit precision: `"[Tên trường] chỉ được nhập tối đa 15 chữ số phần nguyên và 5 chữ số phần thập phân"` (V04)<br>- Quá max length: `"[Tên trường] nhập quá ký tự cho phép"` |
...
| v3 | 2026-05-22 | Antigravity QC Agent | CMR Alignment — Áp dụng các thay đổi sau:<br>1. **Numeric precision (CMR_05 C05b):** Trường số (Cột 7, 8) sửa từ "Max 18 ký tự" → "Phần nguyên tối đa 15 chữ số, phần thập phân tối đa 5 chữ số (tổng 21 ký tự)". Error: `"[Tên trường] chỉ được nhập tối đa 15 chữ số phần nguyên và 5 chữ số phần thập phân"`<br>2. **Placeholder fix (CMR_06 A05):** Sửa placeholder Cột 9 từ `"Nhập Văn bản thu hồi chủ trương ĐT/GCNĐKĐT"` → `"Nhập Văn bản thu hồi"` (rút gọn theo chuẩn ~30 ký tự)<br>3. **Trailing dot removal (CMR H rule):** Bỏ dấu `.` ở cuối tất cả error/toast/validation/tooltip messages<br>4. **Button states (CMR I01):** Xác nhận "Luôn Enabled" cho [Lưu nháp], [Nộp báo cáo], [Hủy] (đã có sẵn trong v2) |
