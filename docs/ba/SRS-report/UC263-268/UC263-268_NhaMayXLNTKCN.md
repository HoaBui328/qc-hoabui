# UC263-268: Tình hình đầu tư và xây dựng nhà máy xử lý nước thải tập trung của các KCN (Biểu 2105)

| Thuộc tính | Giá trị |
| --- | --- |
| **BA phụ trách** | hoai.ho |
| **Phân hệ** | KKT/KCN |
| **Mẫu biểu** | Biểu số: 2105.H.QLKKT |
...
| **Quy tắc sinh mã báo cáo** | EZ_2105HQLKKT_[ID] |
...
> Đặc thù: Form gồm (1) **Header kỳ báo cáo** auto-fill Disabled từ kỳ hạn đã chọn; (2) **Bảng động** 13 cột, mỗi dòng = 1 KCN; (3) **Hiển thị có điều kiện (BR04):** Các cột 11-13 của biểu mẫu (template) — Nguyên nhân, Thời gian dự kiến, Giải pháp xử lý — chỉ hiển thị khi Field 6 (Tình trạng hoạt động) = **"Chưa xây dựng"**. Không có Overview section (dữ liệu định tính, không tổng hợp).

---

## UC263-268.1: Xem Danh Sách Báo Cáo

### 1. Mô tả chức năng

- Tên chức năng: Xem danh sách báo cáo tình hình đầu tư và xây dựng nhà máy XLNT tập trung tại KCN (Biểu 2105)
...
Truy cập chức năng: Phân hệ Báo cáo KKT/KCN → Tình hình đầu tư nhà máy XLNT tập trung KCN (Biểu 2105)

Chức năng đáp ứng usecase số: 263, 264, 265, 266, 267, 268
...
| 7 | Lập báo cáo | Button | Null | | | Chỉ hiển thị khi **Trong thời hạn**. Tham chiếu: CF_01, CMR_18, CMR_04. |
| 8 | Nhập từ file | Button | Null | | | Chỉ hiển thị khi **Trong thời hạn**. Tham chiếu: CF_02, CMR_04. |
| **Bảng Danh sách Báo cáo** | | | | | | |
| 9 | Mã báo cáo | Label | Null | | | EZ_2105HQLKKT_[ID]. Tham chiếu: CMR_09. |
| 10 | Ngày cập nhật | Label | Null | | | Định dạng: dd/MM/yyyy HH:mm. |
| 11 | Trạng thái | Label | Null | | | Tham chiếu: CMR_03. |
| 12 | Hành động | Button group | Null | | | Chi tiết: UC263-268.3. |

---

## UC263-268.2: Lập Báo Cáo

### 1. Mô tả chức năng

- Tên chức năng: Tạo mới Báo cáo tình hình đầu tư và xây dựng nhà máy XLNT tập trung tại KCN (Biểu 2105)
- Chức năng cho phép Cán bộ BQL khai báo thông tin nhà máy XLNT theo từng KCN. Bảng động, mỗi dòng = 1 KCN. Một số cột chỉ hiển thị theo điều kiện.

Phân quyền: Cán bộ BQL các khu công nghiệp, kinh tế. Kỳ hạn phải **Trong thời hạn**. Tham chiếu: CMR_04.

Truy cập chức năng: Màn danh sách báo cáo (UC263-268.1) → Nhấn nút [Lập báo cáo]

Chức năng đáp ứng usecase số: 263, 264
...
| 1 | Quý báo cáo | Label | Auto-fill từ kỳ hạn đã chọn | | | VD: "Quý 1". Disabled. Tham chiếu: CMR_08. |
| 2 | Năm báo cáo | Label | Auto-fill từ kỳ hạn đã chọn | | | VD: "2026". Disabled. Tham chiếu: CMR_08. |
| **PHẦN II — BẢNG ĐỘNG (Add/Remove Row — Tối thiểu 1 dòng khi Nộp)** | | | | | | |
| **Nhóm A — Thông tin KCN & Nhà máy XLNT (Tất cả trạng thái)** | | | | | | |
| 3 | Tên KCN | Single-choice Dropdown | Null | x | x | Danh sách KCN trong hệ thống. Có search. Tham chiếu: CMR_06, CMR_07. |
| 4 | Vị trí KCN | Dropdown | Null | x | x | KCN nằm ngoài KKT / KCN trong KKT ven biển / KCN trong KKT cửa khẩu. Tham chiếu: CMR_07. |
| 5 | Tình trạng quy hoạch NM XLNT | Dropdown | Null | x | x | Có quy hoạch / Dùng chung KCN khác / Không quy hoạch. Tham chiếu: CMR_07. |
| 6 | Tình trạng hoạt động NM XLNT | Dropdown | Null | x | x | Chưa xây dựng / Đang xây dựng / Đang hoạt động. **Trigger BR04:** Khi chọn "Chưa xây dựng" → hiển thị Fields 12-14. Tham chiếu: CMR_07. |
| 7 | Công suất thiết kế | Number | Null | x | x | BR01. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "m3/ngày đêm". Tham chiếu: CMR_06. |
| 8 | Công suất hoạt động | Number | Null | x | x | BR01. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "m3/ngày đêm". Tham chiếu: CMR_06. |
| 9 | Chất lượng nước thải (đạt/không đạt) | Radio button | Null | x | x | Đạt tiêu chuẩn môi trường (lưu: 1) / Không đạt (lưu: 0). |
| 10 | Chất lượng nước thải (phân loại cột) | Dropdown | Null | x | x | Cột A / Cột B / Chưa xác định. Tham chiếu: CMR_07. |
| 11 | Tình trạng lắp đặt hệ thống quan trắc tự động | Dropdown | Null | x | x | Chưa lắp đặt / Đang xây dựng / Đã vận hành. Tham chiếu: CMR_07. |
| **Nhóm B — Hiển thị có điều kiện: Field 6 = "Chưa xây dựng" (BR04)** | | | | | | |
| 12 | Nguyên nhân chưa có NM XLNT tập trung | Textarea | Null | x | x* | Max 3000 ký tự. Bắt buộc khi hiển thị. Tham chiếu: CMR_06. Placeholder: "Nhập Nguyên nhân chưa có NM XLNT tập trung". |
| 13 | Thời gian dự kiến đưa vào vận hành | Textbox | Null | x | x* | Định dạng tháng/năm (VD: 06/2026). Max 255 ký tự. Bắt buộc khi hiển thị. Tham chiếu: CMR_06. Placeholder: "Nhập Thời gian dự kiến đưa vào vận hành". |
| 14 | Giải pháp xử lý đảm bảo quy định BVMT | Textarea | Null | x | x* | Max 3000 ký tự. Bắt buộc khi hiển thị. Tham chiếu: CMR_06. Placeholder: "Nhập Giải pháp xử lý đảm bảo quy định BVMT". |
| **Các Button** | | | | | | |
| 15 | Hủy | Button | | | | Luôn Enabled. Tham chiếu: CF_01, CMR_18. |
| 16 | Xem trước | Button | | | | Luôn Enabled. Popup PDF Preview. Tham chiếu: CF_07.1. |
| 17 | Lưu nháp | Button | | | | Luôn Enabled. Tham chiếu: CF_01, CMR_18. |
| 18 | Nộp báo cáo | Button | | | | Validate: tối thiểu 1 dòng. Tham chiếu: CF_01, CMR_18. |
...
**Quản lý dòng bảng:**
- Nhấn `[+ Thêm dòng]` để thêm 1 KCN.
- Nút `[Xóa]` ở đầu mỗi dòng. Ẩn khi bảng chỉ còn 1 dòng.
- Validate tối thiểu 1 dòng khi Nộp.

**Auto-fill Header Kỳ báo cáo:** Quý/Năm tự động điền từ kỳ hạn đã chọn ở màn danh sách. Disabled. Tham chiếu: CMR_08.

**Xử lý hiển thị có điều kiện (BR04 — Soft-hide):**
- Trigger: Field 6 (Tình trạng hoạt động NM XLNT).
- Khi Field 6 = **"Chưa xây dựng"**: Hiển thị và bật Required cho Fields 12, 13, 14.
- Khi Field 6 ≠ "Chưa xây dựng": **Ẩn Fields 12, 13, 14 trên giao diện** và tắt chế độ Required. **Dữ liệu đã nhập vẫn được giữ ngầm trong các ô input** (không xóa), cho phép phục hồi khi user chuyển lại về "Chưa xây dựng".
- Hiệu lực ngay khi người dùng thay đổi dropdown (real-time, không cần nhấn nút).
- **Logic submit/save:** Tại thời điểm nhấn [Nộp báo cáo] hoặc [Lưu nháp], hệ thống kiểm tra Field 6. Nếu Field 6 ≠ "Chưa xây dựng", hệ thống tự động **loại bỏ (nullify)** giá trị của Fields 12, 13, 14 trước khi gửi về Database, bất kể trong ô đó đang chứa gì.

**Validate Required:** Tham chiếu CMR_06. Khi ấn CTA mà trường Required để trống → Highlight đỏ và hiển thị lỗi chuẩn CMR_06.

---

## UC263-268.3: Các Tác Vụ Bổ Trợ
...
Truy cập chức năng: Màn danh sách báo cáo (UC263-268.1) → Cột Hành động

Chức năng đáp ứng usecase số: 265, 266, 267, 268
...
| 3 | Xem chi tiết | Button | Tất cả trạng thái | Người tạo | Tham chiếu: CF_07. Hiển thị đúng trạng thái ẩn/hiện của Fields 12-14 theo BR04. |
...
- Xem chi tiết (CF_07): Toàn bộ bảng hiển thị Read-only. Các ô Fields 12-14 chỉ hiển thị với các dòng có Field 6 = "Chưa xây dựng".
- Chỉnh sửa: Tham chiếu CF_03. BR04 vẫn áp dụng khi chỉnh sửa.
...
| 2026-05-22 | Align_CMR | Textbox max length | Max 50 ký tự | Max 255 ký tự | Align Textbox max length theo chuẩn CMR_06 |
| 2026-05-22 | Align_CMR | Textarea max length | Max 1000 ký tự | Max 3000 ký tự | Align Textarea max length theo chuẩn CMR_06 |
...
#### 2.3. Text Validation — Cập nhật max length

| Trường | Max length cũ | Max length mới | Message lỗi |
| :--- | :--- | :--- | :--- |
| Thời gian dự kiến hoàn thành (textbox) | 50 | **255** | `"Thời gian dự kiến hoàn thành không được vượt quá 255 ký tự!"` |
| Quy mô công nghệ (textbox) | Chưa khai báo | **255** | `"Quy mô công nghệ không được vượt quá 255 ký tự!"` |
| Tất cả textarea fields | 1000 | **3000** | `"[Tên trường] không được vượt quá 3000 ký tự!"` |

#### 2.4. V16 — Bảng động rỗng khi Nộp

| Trường hợp | Message lỗi |
| :--- | :--- |
| Bảng động có 0 dòng dữ liệu khi nhấn Nộp | `"Vui lòng khai báo ít nhất 1 nhà máy XLNT trong kỳ báo cáo"` |
...
Áp dụng cho **TẤT CẢ** trường số trong UC263-268:
...
| CMR_06 | Quy tắc trường text (max 255 textbox / 3000 textarea) |
