# UC269-274: Số lao động trực tiếp làm việc tại các khu công nghiệp (Biểu 2106)

| Thuộc tính | Giá trị |
| --- | --- |
| **BA phụ trách** | hoai.ho |
| **Phân hệ** | KKT/KCN |
| **Mẫu biểu** | Biểu số: 2106.H.QLKKT |
...
| **Quy tắc sinh mã báo cáo** | EZ_2106HQLKKT_[ID] |
...
> Đặc thù: Form gồm (1) **Header kỳ báo cáo** auto-fill Disabled; (2) **Bảng động**, mỗi dòng = 1 KCN; (3) **BR04 Cross-validate:** Có 2 cột tự tính (auto-calc, Disabled): `Tổng lao động = Nam + Nữ` và `Tổng theo trình độ = SUM(8 trình độ)`. Khi 2 cột này không bằng nhau → hiển thị icon ⚠️ cạnh cột "Tổng theo trình độ". **Không chặn lưu nháp hay nộp**; (4) **Overview section** 10 trường SUM cuối trang.

---

## UC269-274.1: Xem Danh Sách Báo Cáo

### 1. Mô tả chức năng

- Tên chức năng: Xem danh sách báo cáo số lao động trực tiếp làm việc tại các KCN (Biểu 2106)
...
Truy cập chức năng: Phân hệ Báo cáo KKT/KCN → Số lao động trực tiếp tại KCN (Biểu 2106)

Chức năng đáp ứng usecase số: 269, 270, 271, 272, 273, 274
...
| 9 | Mã báo cáo | Label | Null | | | EZ_2106HQLKKT_[ID]. Tham chiếu: CMR_09. |
| 10 | Ngày cập nhật | Label | Null | | | Định dạng: dd/MM/yyyy HH:mm. |
| 11 | Trạng thái | Label | Null | | | Tham chiếu: CMR_03. |
| 12 | Hành động | Button group | Null | | | Chi tiết: UC269-274.3. |

---

## UC269-274.2: Lập Báo Cáo

### 1. Mô tả chức năng

- Tên chức năng: Tạo mới Báo cáo số lao động trực tiếp tại KCN (Biểu 2106)
- Chức năng cho phép Cán bộ BQL khai báo số liệu lao động theo từng KCN. Bảng động, mỗi dòng = 1 KCN. Hệ thống tự kiểm tra tính nhất quán giữa các nhóm lao động (BR04).

Phân quyền: Cán bộ BQL các khu công nghiệp, kinh tế. Kỳ hạn phải **Trong thời hạn**. Tham chiếu: CMR_04.

Truy cập chức năng: Màn danh sách báo cáo (UC269-274.1) → Nhấn nút [Lập báo cáo]

Chức năng đáp ứng usecase số: 269, 270
...
| 3 | Tên KCN | Single-choice Dropdown | Null | x | x | Danh sách KCN. Có search. Tham chiếu: CMR_07. |
| 4 | Vị trí KCN | Dropdown | Null | x | x | KCN nằm ngoài KKT / KCN trong KKT ven biển / KCN trong KKT cửa khẩu. Tham chiếu: CMR_07. |
| 5 | Tổng số lao động trực tiếp | Label (Disabled) | Tự tính | | | Auto-calc = Field 6 + Field 7. Real-time. Label: "người". |
| **Giới tính** | | | | | | |
| 6 | Số lao động Nam | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 7 | Số lao động Nữ | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| **Trình độ học vấn** | | | | | | |
| 8 | Phổ thông (Tiểu học, THCS, THPT) | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 9 | Sơ cấp | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 10 | Trung cấp | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 11 | Cao đẳng | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 12 | Đại học | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 13 | Trên đại học | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| 14 | Khác | Number | Null | x | x | Số nguyên ≥ 0. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Label: "người". Tham chiếu: CMR_06. |
| **BR04 — Kiểm tra nhất quán** | | | | | | |
| — | ~~Tổng theo trình độ~~ | ~~Label (Disabled)~~ | ~~Tự tính~~ | | | **ĐÃ BỎ.** Tổng số lao động trực tiếp chỉ tính bằng Field 6 + Field 7 (Nam + Nữ). Không còn cross-validate với SUM(trình độ). |
| **PHẦN III — OVERVIEW (Cuối trang, Disabled, auto-calc)** | | | | | | |
| 15 | Tổng số lao động toàn bộ KCN | Label (Disabled) | Tự tính | | | SUM(Field 5). Label: "người". |
| 16 | Tổng lao động Nam | Label (Disabled) | Tự tính | | | SUM(Field 6). Label: "người". |
| 17 | Tổng lao động Nữ | Label (Disabled) | Tự tính | | | SUM(Field 7). Label: "người". |
| 18 | Tổng trình độ Phổ thông | Label (Disabled) | Tự tính | | | SUM(Field 8). Label: "người". |
| 19 | Tổng trình độ Sơ cấp | Label (Disabled) | Tự tính | | | SUM(Field 9). Label: "người". |
| 20 | Tổng trình độ Trung cấp | Label (Disabled) | Tự tính | | | SUM(Field 10). Label: "người". |
| 21 | Tổng trình độ Cao đẳng | Label (Disabled) | Tự tính | | | SUM(Field 11). Label: "người". |
| 22 | Tổng trình độ Đại học | Label (Disabled) | Tự tính | | | SUM(Field 12). Label: "người". |
| 23 | Tổng trình độ Trên đại học | Label (Disabled) | Tự tính | | | SUM(Field 13). Label: "người". |
| 24 | Tổng trình độ Khác | Label (Disabled) | Tự tính | | | SUM(Field 14). Label: "người". |
| **Các Button** | | | | | | |
| 25 | Hủy | Button | | | | Luôn Enabled. Tham chiếu: CF_01. |
| 26 | Xem trước | Button | | | | Luôn Enabled. Popup PDF Preview. Tham chiếu: CF_07.1. |
| 27 | Lưu nháp | Button | | | | Luôn Enabled. Tham chiếu: CF_01. |
| 28 | Nộp báo cáo | Button | | | | Validate: tối thiểu 1 dòng. Tham chiếu: CF_01. |
...
**Quản lý dòng bảng:**
- Nhấn `[+ Thêm dòng]` để thêm 1 KCN. Nút `[Xóa]` ở đầu mỗi dòng, ẩn khi bảng chỉ còn 1 dòng.
...
**Auto-calculate Field 5 (Tổng số lao động):** Real-time = Field 6 + Field 7. Disabled.

**Overview Section:** Disabled, cập nhật real-time theo dữ liệu bảng.

---

## UC269-274.3: Các Tác Vụ Bổ Trợ
...
Truy cập chức năng: Màn danh sách báo cáo (UC269-274.1) → Cột Hành động

Chức năng đáp ứng usecase số: 271, 272, 273, 274
...
- Xem chi tiết (CF_07): Bảng Read-only. Overview section hiển thị ở cuối trang.
- Nộp từ Màn hình danh sách (CF_09): Tham chiếu CF_09. BR04 chỉ cảnh báo icon, không chặn nộp.
...
Áp dụng cho **TẤT CẢ** trường số trong UC269-274:
