...
|---|------------------|------------------|------|----------|---------|-------------|-------------|--------------------------|--------|
| 1 | Khung Lọc & Tìm kiếm | Thanh tìm kiếm | Search bar | No | Null | "Tìm kiếm nhanh theo Mã báo cáo, Tên dự án" | N/A | Tìm theo Mã báo cáo, Tên dự án. Kết quả hiển thị ngay khi nhập. CMR_06, CMR_09. Max 255 ký tự | UC spec |
...
| 66 | Phần C | Khó khăn, vướng mắc, kiến nghị (C-001) | Textarea | No | — | "Nhập Khó khăn, vướng mắc, kiến nghị" | N/A | Optional. Hỗ trợ xuống dòng. Max 3000 ký tự. Error: "Khó khăn, vướng mắc, kiến nghị không được vượt quá 3000 ký tự!" | UC spec |
...
|--------------------|---------------|--------------------|---------------------------------------------|
| Thanh tìm kiếm | Enabled. Empty by default | Type: filter ngay khi nhập (real-time). Clear: reset filter | Kết quả hiển thị ngay. "Không tìm thấy kết quả" nếu empty. Max 255 ký tự |
...
| eForm Grid cells (Editable) | Enabled. Nền trắng | Type: nhập số. Blur: auto-round 5 decimal | CMR_05. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số (tổng 21 ký tự). Error: "[Tên trường] chỉ được nhập tối đa 15 chữ số phần nguyên và 5 chữ số phần thập phân" |
...
| Field / Object | Required | Format / Constraint | Min / Max | Error Message |
|----------------|----------|---------------------|-----------|---------------|
| Editable cells (Phần B) | Yes | Number. Phần nguyên max 15 chữ số, phần thập phân max 5 chữ số. Auto-round half-up | — / 21 ký tự (15+1+5) | "[Tên trường] chỉ được nhập tối đa 15 chữ số phần nguyên và 5 chữ số phần thập phân" |
...
| Data Integrity | Max length: Textbox 255, Textarea 3000, Number 21 (15+1+5), Search 255 | CMR_05/CMR_06 (aligned 2026-05-22) |
...
| v3.0 | 2026-05-22 | Claude Agent | Align CMR: Search max 200→255, Textarea max 2000→3000, Numeric max 20→21 (15+5), chuẩn hóa placeholder, bỏ dấu `.` cuối empty state, cập nhật error message format |
