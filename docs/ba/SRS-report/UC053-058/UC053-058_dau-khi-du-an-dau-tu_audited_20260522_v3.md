# UC Readiness Review — UC053-058 (v3)
...
| 4 | Khung Lọc | Mã báo cáo | Search bar | No | Null | "Tìm kiếm nhanh theo mã báo cáo" | N/A | Tìm theo mã BC. Max 255 ký tự. CMR_06, CMR_09 | UC spec + UI |
...
| 18 | Grid Nhóm I | (4) Tên dự án | API Label / Editable | Yes (Thêm nhanh) | API auto-fill | — | N/A | Read-only khi chọn API (Disabled). Editable khi "Thêm nhanh" hoặc API fail. Max 255 ký tự. Frozen. RULE-01. CMR_06, CMR_12 | UC spec + UI |
| 19 | Grid Nhóm I | (5) Địa điểm dự án | API Label / Editable | No | API auto-fill | — | N/A | Read-only khi chọn API (Disabled). Editable khi "Thêm nhanh" hoặc API fail. Max 255 ký tự. CMR_06, CMR_12 | UC spec |
...
| 30 | Grid Nhóm I | (16) Tình hình hoạt động | Editable (Textbox) | No | Null | "Nhập tình hình hoạt động" | N/A | Optional. Max 255 ký tự. CMR_06 | UC spec |
...
| 37 | Grid Nhóm II | (16) Tình hình hoạt động | Editable (Textbox) | No | Null | "Nhập tình hình hoạt động" | N/A | Optional. Max 255 ký tự | UC spec |
...
| Search Mã BC | Enabled. Null | Type: filter real-time | Max 255 ký tự. CMR_06, CMR_09 |
...
| Cột (4) Tên dự án | Disabled (API) / Enabled (Thêm nhanh / API fail). Frozen | N/A khi Disabled. Type khi Enabled | RULE-01 Frozen. Bắt buộc khi Thêm nhanh. Max 255 ký tự. CMR_06, CMR_12 |
| Cột (5) Địa điểm | Disabled (API) / Enabled (Thêm nhanh / API fail) | N/A khi Disabled. Type khi Enabled | Optional. Max 255 ký tự. CMR_06, CMR_12 |
...
| Cột (16) Tình hình hoạt động | Enabled. Optional | Type: text | Max 255 ký tự. Placeholder: "Nhập tình hình hoạt động" |
...
| Max length | — | Textbox 255, Textarea 3000, Number 21 (15 nguyên + 1 dấu chấm + 5 thập phân), Search 255 | — | — |
...
| Data Integrity | Max length: Textbox 255, Textarea 3000, Number 21 (15 nguyên + 1 dấu chấm + 5 thập phân), Search 255 | CMR rules |
...
| v3.0 | 2026-05-22 | QC Agent | CMR alignment: Search max 200→255, Textbox max 500→255, Number max 20→21 (15 nguyên + 1 dấu chấm + 5 thập phân), NFR summary updated. Placeholder "Nhập dữ liệu" → placeholder cụ thể theo tên trường. Địa điểm dự án max 200→255. |
