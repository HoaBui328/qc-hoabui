...
**Ngày tạo:** 29/04/2026
**Phiên bản:** v2.5
**Cập nhật:** 14/05/2026 — Đồng bộ §3.1 Main Flow với §2.2: bổ sung đầy đủ inline error cases, phản ánh on-blur validation và CMR-09 Disabled/Enabled
...
| 2 | Mã định danh                    | Textbox          | **Quy tắc hiển thị:**<br>- Label: **"Mã định danh*"** (có dấu * indicating bắt buộc).<br>- Placeholder: **"Nhập mã định danh của bạn"**.<br>- Có icon thẻ (card) ở đầu trường nhập.<br>- Bàn phím: Numeric.<br><br>**Quy tắc hành động:**<br>- Tap vào trường → Mở bàn phím numeric để nhập.<br>- Hệ thống auto-trim khoảng trắng đầu/cuối trước khi validate (CMR-09).<br><br>**Validation** *(hiển thị lỗi ngay khi rời khỏi trường — tap sang trường khác hoặc tap ra ngoài)*:<br>- Để trống hoặc chỉ khoảng trắng → *"Vui lòng nhập Mã định danh"*<br>- Nhập ít hơn 10 số → *"Sai định dạng."*<br>- Nhập quá 13 số → **Block nhập** (hệ thống không cho phép nhập quá 13 ký tự số).<br>- Nhập ký tự không phải số (chữ cái, ký tự đặc biệt) → *"Mã định danh chỉ gồm các số 0-9."*<br>- Có dấu cách hoặc dấu gạch ngang → *"Mã định danh chỉ gồm các số 0-9."* |
| 3 | Mật khẩu                         | Password Input   | **Quy tắc hiển thị:**<br>- Label: **"Mật khẩu*"** (có dấu * indicating bắt buộc).<br>- Placeholder: **"Nhập mật khẩu"**.<br>- Có icon mắt (ẩn/hiện) bên phải trường nhập.<br>- Max length: **50 ký tự**.<br><br>**Quy tắc hành động:**<br>- Tap icon mắt → Toggle ẩn/hiện mật khẩu.<br>- **Không** auto-trim khoảng trắng (theo CMR-09 — password field được loại trừ).<br><br>**Validation** *(hiển thị lỗi ngay khi rời khỏi trường)*:<br>- Để trống → *"Vui lòng nhập Mật khẩu"*                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 8 | Nút "Đăng nhập"                | Button (Primary) | **Quy tắc hiển thị:**<br>- Nền đỏ đậm, text trắng.<br>- **Sticky Bottom**: cố định dưới cùng màn hình, không cuộn theo nội dung.<br>- **Disabled** (nền xám, không tương tác) khi ít nhất một trường bắt buộc trống hoặc có lỗi validation. **Enabled** (nền đỏ) khi tất cả trường bắt buộc hợp lệ. (CMR-09)<br><br>**Quy tắc hành động:**<br>- Tap (khi Enabled) → Hiển thị loading overlay → Gọi API đăng nhập.<br>- Tap (khi Disabled) → Không có hành động.<br>- Có debounce tránh double-tap (CMR-18).                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
1. Người dùng nhập Mã định danh và Mật khẩu. Validation hiển thị **on-blur** (ngay khi rời khỏi trường — CMR-09):
   ...
   - Mã định danh ít hơn 10 chữ số → Inline error: *"Sai định dạng."*
   - Mã định danh quá 13 chữ số → **Block nhập** (hệ thống không cho phép nhập quá 13 ký tự số).
...
2. Nút "Đăng nhập" **Disabled** khi bất kỳ trường nào có lỗi; tự động **Enabled** khi tất cả trường bắt buộc hợp lệ (CMR-09).
...
| AC-02-2  | Validation Mã định danh - ít hơn 10 số           | Nhập 9 số → focus out →*"Sai định dạng."*                                     |
| AC-02-3  | Validation Mã định danh - block nhập quá 13 số   | Nhập 13 số → ký tự thứ 14 bị block, hệ thống không nhận thêm ký tự.           |
...
| AC-02-14 | Khóa tài khoản                                    | 5 lần sai → Toast: *"Tài khoản đã bị tạm khóa. Vui lòng thử lại sau 15 phút."* |
...
| AC-02-17 | CMR-09: Nút Đăng nhập Disabled                    | Khi form chưa hợp lệ (trường bắt buộc trống / có lỗi) → Nút Disabled (nền xám, không tương tác). |
| AC-02-18 | CMR-09: Nút Đăng nhập Enabled                     | Khi tất cả trường bắt buộc hợp lệ → Nút Enabled (nền đỏ, có thể tap). |
| AC-02-19 | Max 50 ký tự Mật khẩu                             | Nhập ký tự thứ 51 → Hệ thống không nhận thêm ký tự (field bị cap ở 50). |
