...
| - | ------------- | -------------------- | ------------------------------------------- | ------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Ô tìm kiếm | Textbox (Search)     | Placeholder: "Tìm kiếm nhanh theo tên lô đất" | x            | -         | **Quy tắc hiển thị:**<br>- Tìm kiếm like theo tên lô đất.<br>- Tìm kiếm gần đúng (chứa từ khóa). Khi xóa hết từ khóa, danh sách trở về trạng thái mặc định (hiển thị tất cả).<br><br>**Quy tắc hành vi:**<br>- Debounce 3 giây: hiển thị kết quả tìm kiếm trong lúc gõ, sau 3 giây không gõ mới thực hiện tìm kiếm.<br>- Người dùng nhập từ khóa → Kết quả hiển thị ngay (sau debounce), không cần nhấn Enter hay nút Tìm kiếm.<br>- Tự động trim space (cắt khoảng trắng hai đầu) trước khi tìm kiếm. Nếu sau trim rỗng → không trigger search, danh sách về mặc định.<br>- Nếu không có kết quả trùng khớp: Hiển thị *"Không tìm thấy kết quả."* ở giữa vùng nội dung (Xem CMR-14). Không có action Thử lại vì không phải lỗi.<br>- Max length: **255 ký tự** (Xem CMR-01). Khi nhập đủ 255 ký tự, không cho phép nhập thêm.<br>- Xem CMR-01 |
...
| # | Tên trường           | Kiểu trường                          | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| - | ----------------------- | --------------------------------------- | --------------------- | ------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Khu công nghiệp       | Dropdown (Single-selection, Searchable) | Tất cả | x            | -         | **Quy tắc hiển thị:**<br>- Placeholder: "Chọn khu công nghiệp". Hiển thị placeholder khi option được clear.<br>- Danh sách các KCN lấy từ API. Mặc định: "Tất cả".<br>- **Searchable dropdown**: Cho phép người dùng nhập từ khóa để tìm kiếm trong danh sách KCN (tìm kiếm gần đúng, sắp xếp A-Z). Chi tiết xem CMR-03.<br><br>**Quy tắc hành động:**<br>- Chọn một KCN cụ thể để lọc lô đất chỉ thuộc KCN đó. Giá trị "Tất cả" là tùy chọn mặc định — nghĩa là không áp dụng lọc theo KCN.<br>- Tap → Mở danh sách lựa chọn (có ô search) → Nhập từ khóa để lọc option → Tap item → Tự động đóng và hiển thị giá trị đã chọn.<br>- Option đã chọn được highlight/bold khi mở lại dropdown. (Xem CMR-03)<br>- Nếu đang ở trạng thái clear (hiển thị placeholder) mà người dùng tap ra ngoài → tự động chọn lại "Tất cả".               |
| 2 | Diện tích (Từ)       | Số (m²)                               | Placeholder: "từ"    | x            | -         | **Quy tắc hiển thị:**<br>- Placeholder mặc định là "từ".<br>- Nếu để trống: mặc định là tất cả (không lọc).<br><br>**Quy tắc hành động:**<br>- Nhập giá trị diện tích tối thiểu (m²).<br>- Cho phép nhập **số thập phân**. Chỉ cho phép ký tự 0-9 và dấu chấm "."; **block** ký tự "-" và các ký tự khác.<br>- Định dạng số: 3 chữ số phần nguyên phân cách bằng dấu phẩy (,), phần thập phân phân cách bằng dấu chấm (.) (Xem CMR-11).<br><br>**Quy tắc validation:**<br>- Giá trị phải > 0 (không cho phép số âm hoặc 0).                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 3 | Diện tích (Đến)     | Số (m²)                               | Placeholder: "đến"  | x            | -         | **Quy tắc hiển thị:**<br>- Placeholder mặc định là "đến".<br>- Nếu để trống: mặc định là tất cả (không lọc).<br><br>**Quy tắc hành động:**<br>- Nhập giá trị diện tích tối đa (m²).<br>- Cho phép nhập **số thập phân**. Chỉ cho phép ký tự 0-9 và dấu chấm "."; **block** ký tự "-" và các ký tự khác.<br>- Định dạng số: 3 chữ số phần nguyên phân cách bằng dấu phẩy (,), phần thập phân phân cách bằng dấu chấm (.) (Xem CMR-11).<br><br>**Quy tắc validation:**<br>- Giá trị phải > 0 (không cho phép số âm hoặc 0).<br>- Diện tích (Đến) phải lớn hơn Diện tích (Từ). Nếu diện tích đến nhỏ hơn diện tích từ, hiển thị inline error message "Diện tích (Đến) phải lớn hơn Diện tích (Từ)".                                                                                                                                                                 |
...
| 7 | Nút "Áp dụng"        | Button (Primary)                        | -                    | —           | -         | **Quy tắc hiển thị:**<br>- **Luôn Enabled** (CMR-09 I01). Không có trạng thái Disabled.<br><br>**Quy tắc hành động:**<br>- Tap → Validate toàn bộ tiêu chí lọc. Nếu validation Diện tích fail (Đến < Từ) → Hiển thị inline error *"Diện tích (Đến) phải lớn hơn Diện tích (Từ)"*, không đóng Bottom Sheet.<br>- Nếu hợp lệ → Áp dụng toàn bộ tiêu chí lọc, đóng Bottom Sheet, gọi API với tham số bộ lọc, tải lại danh sách lô đất. |
...
| 4 | Diện tích             | Label           | -                    | —           | -         | **Quy tắc hiển thị:**<br>- Diện tích lô đất (đơn vị m²). Cho phép giá trị **số thập phân**.<br>- **Layout:** Label bên trái, value căn phải.<br>- Định dạng số: 3 chữ số phần nguyên phân cách bằng dấu phẩy (,), phần thập phân phân cách bằng dấu chấm (.) (VD: "50,000.5", "12,000.25"). Áp dụng CMR-11.<br>- Không có icon ở prefix.              |
...
| 7 | Card lô đất         | Card            | -                    | —           | -         | **Quy tắc hành động:**<br>- Tap bất kỳ vị trí nào trên card → mở màn hình Chi tiết lô đất.<br>- Debounce navigation theo CMR-18: khi người dùng tap nhanh liên tục (double tap) vào card, hệ thống có cơ chế debounce để chỉ trigger 1 lần mở chi tiết, tránh mở trùng lặp màn hình.                                                                                                                                                                         |
...
| 4 | Diện tích       | Label (Number) | -                    | —           | -         | **Quy tắc hiển thị:**<br>- Diện tích lô đất (đơn vị m²). Cho phép giá trị **số thập phân**.<br>- Định dạng số: 3 chữ số phần nguyên phân cách bằng dấu phẩy (,), phần thập phân phân cách bằng dấu chấm (.) (VD: "50,000.5", "12,000.25"). Áp dụng CMR-11.<br>- Nếu null → "-".<br>- Nếu dài quá → xuống dòng (wrap text), không truncate.<br><br>**Quy tắc hành động:**<br>- Không tap được. |
...
| # | Tên trường       | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                                                                                                                      |
| - | ------------------- | -------------- | --------------------- | ------------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Thông tin chi tiết | Label (Multi-line) | -                    | —           | -         | **Quy tắc hiển thị:**<br>- Hiển thị thông tin chi tiết của lô đất.<br>- Dữ liệu lấy từ API.<br>- Nếu null → "-".<br>- Nếu dài quá → xuống dòng (wrap text), không truncate.<br><br>**Quy tắc hành động:**<br>- Không tap được. |
...
- Nếu không có lô đất nào trong hệ thống: Hiển thị *"Không có dữ liệu."* ở giữa vùng nội dung (Xem CMR-14). Không có action Thử lại vì không phải lỗi.
...
4. Nếu không có kết quả phù hợp: Hiển thị *"Không tìm thấy kết quả."* ở giữa vùng nội dung (Xem CMR-14). Không có action Thử lại vì không phải lỗi.
...
| ---------- | ----------- | ------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| 2026-04-29 | v1          | Khởi tạo tài liệu           | -            | Tạo đặc tả UC40 — Tra cứu thông tin quỹ đất trong KCN trên Mobile: danh sách lô đất, bộ lọc, chi tiết lô đất | Phiên bản đầu tiên |
...
| 2026-05-08 | v2.1 → v3 | Section 2.1 — Ô tìm kiếm (Max length) | (Không có) | Max length: 255 ký tự (Xem CMR-01) | Bổ sung max length theo CMR-01 v1.1 |
...
| 2026-05-15 | v3.1 → v3.2 | Section 2.1 — Nút "Lọc" (filter indicator) | "badge đếm số filter đang bật" | Icon indicator màu xanh lá cây ở góc phải bên trên icon filter theo CMR-02 | Đồng bộ với UC2 theo CMR-02 |
| 2026-05-15 | v3.1 → v3.2 | Section 2.1 — Ô tìm kiếm (trim space) | (Không có) | Tự động trim space trước khi tìm kiếm; nếu sau trim rỗng → không trigger search | Đồng bộ với UC2 |
| 2026-05-15 | v3.1 → v3.2 | Section 2.1 — Ô tìm kiếm (empty state) | Hiển thị màn "Không tìm thấy kết quả." | Hiển thị "Không tìm thấy kết quả." ở giữa vùng nội dung. Không có action Thử lại vì không phải lỗi | Đồng bộ với UC2 theo CMR-14 |
| 2026-05-15 | v3.1 → v3.2 | Section 5.1 — Lazy load (hết dữ liệu) | "Khi không còn dữ liệu, ẩn loading indicator" | "Khi hết dữ liệu, ẩn loading indicator và không gọi API nữa" + tham chiếu CMR-04 | Đồng bộ với UC2 |
| 2026-05-15 | v3.1 → v3.2 | Section 2.1 & 5.2 — State Persistence | Giữ trạng thái tìm kiếm/lọc | Giữ trạng thái tìm kiếm/lọc/scroll + dữ liệu đã load trước đó (Xem CMR-01) | Đồng bộ với UC2 |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Bộ lọc Khu công nghiệp (mặc định) | "Tất cả các KCN" + mô tả dài | "Tất cả" — chỉ ghi danh sách KCN lấy từ API, mặc định "Tất cả" | BA feedback rút gọn mô tả |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Bộ lọc Diện tích (Từ/Đến) | Chỉ cho phép số nguyên dương | Cho phép **số thập phân** (0-9 và dấu "."); block "-" và ký tự khác; định dạng theo CMR-11 | BA feedback cho phép số thập phân |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Bộ lọc Tình trạng quỹ đất (mặc định) | "Tất cả" | "Tất cả" | BA feedback đổi tên giá trị mặc định |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Bộ lọc Tình trạng công bố (mặc định) | "Tất cả" | "Tất cả" | BA feedback đổi tên giá trị mặc định |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Card lô đất (rule click) | (Không có) | Bổ sung row "Card lô đất": Tap bất kỳ vị trí nào trên card → mở Chi tiết lô đất; debounce navigation theo CMR-18 | Đồng bộ với UC2 |
| 2026-05-16 | v3.2 → v3.3 | Section 2.1 — Diện tích trong card | "VD: 50,000, 12,000" + chỉ phân tách hàng nghìn | Cho phép **số thập phân**; định dạng 3 chữ số phần nguyên phân cách dấu phẩy, phần thập phân phân cách dấu chấm (VD: "50,000.5", "12,000.25") theo CMR-11 | BA feedback diện tích là số thập phân |
| 2026-05-16 | v3.2 → v3.3 | Section 2.2.2 — Diện tích trong màn chi tiết | Hiển thị format hàng nghìn (VD: "50,000") | Cho phép **số thập phân**; định dạng theo CMR-11 (VD: "50,000.5", "12,000.25") | BA feedback diện tích là số thập phân |
| 2026-05-16 | v3.2 → v3.3 | Section 2.2.5 — Thông tin chi tiết | Tên trường: "Loại đất" — hiển thị loại đất (VD: "Đất nông nghiệp") | Tên trường: "Thông tin chi tiết" — hiển thị thông tin chi tiết của lô đất (Label Multi-line, lấy từ API) | BA feedback đổi từ "Loại đất" sang "Thông tin chi tiết" |
| 2026-05-16 | v3.2 → v3.3 | Section 2.2.6 — File đính kèm (thứ tự sắp xếp) | "Cùng thời điểm → theo thứ tự tạo" | "Cùng lúc → hiển thị theo thứ tự như lúc tạo/chỉnh sửa" | BA feedback rõ thứ tự khi upload cùng lúc |
| 2026-05-16 | v3.2 → v3.3 | Section 2.2.6 — File đính kèm (Quy tắc hành động) | Chỉ ghi PDF/Image/Video → mở trên trình duyệt | Bổ sung đầy đủ theo CMR-08: Xem trực tiếp (PDF, JPG, PNG, MP4, AVI, MOV) / Tải xuống (DOC, DOCX, XLS, XLSX, ZIP...) / Không hỗ trợ → toast "Định dạng không hỗ trợ. Vui lòng tải xuống." | Đồng bộ đầy đủ với CMR-08 |
| 2026-05-20 | v3.3 → v3.4 | Section 2.1 — Placeholder dropdown KCN (row 1) | (Không có placeholder) | Thêm Placeholder: "Chọn khu công nghiệp" | BA feedback — thêm placeholder cho dropdown searchable |
| 2026-05-20 | v3.4 → v3.5 | Section 2.1 — Dropdown KCN (row 1): hành vi clear | (Không có) | Hiển thị placeholder khi option được clear; nếu đang clear mà tap ra ngoài → auto chọn lại "Tất cả" | BA feedback — bổ sung hành vi clear/focus-out |
| 2026-05-21 | v3.5 → v3.6 | Align CMR Mobile v6.0 | 1. Search box max length 500→255. 2. Nút "Áp dụng" Disabled→Luôn Enabled. 3. Bổ sung CMR-03 cho Searchable Dropdown. |
| 2026-05-21 | v3.6 → v3.7 | Dropdown filter | Giá trị mặc định "Tất cả [tên trường]"→"Tất cả". |
| 2026-05-21 | v3.7 → v3.8 | §2.1 #7 — Nút "Áp dụng" (body) | `Disabled (grayed out) khi validation fail` | **Luôn Enabled** (CMR-09 I01). Tap → validate → inline error nếu fail, không đóng sheet | Đồng bộ body với lịch sử v3.6 — body chưa được cập nhật |
