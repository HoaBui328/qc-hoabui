...
Mô tả giao diện: Bộ lọc hiển thị dưới dạng Bottom Sheet. Người dùng có thể lọc danh sách lịch hẹn theo lĩnh vực và thủ tục hành chính. Hai trường Lĩnh vực và Thủ tục hành chính **không phụ thuộc vào nhau** (chọn lĩnh vực không ảnh hưởng đến danh sách thủ tục hành chính và ngược lại). Sau khi chọn tiêu chí lọc, người dùng tap "Áp dụng" để tải lại danh sách phù hợp.

| # | Tên trường     | Kiểu trường              | Giá trị mặc định      | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| - | ----------------- | --------------------------- | -------------------------- | ------------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Lĩnh vực        | Dropdown (Single-selection) | "Tất cả"      | x            | —         | **Quy tắc hiển thị:** (Xem CMR-03)<br>- Placeholder: "Chọn lĩnh vực".<br>- Danh sách lĩnh vực lấy từ danh mục hệ thống.<br>- Giá trị mặc định: "Tất cả".<br><br>**Quy tắc hành động:** (Xem CMR-02, CMR-03)<br>- Người dùng tap → Mở danh sách lựa chọn → Tap item → Tự động đóng và hiển thị giá trị đã chọn.<br>- Người dùng có thể nhập text để tìm kiếm/filter trong dropdown (tìm kiếm gần đúng).<br>- Option đã chọn được highlight/bold khi mở dropdown list.<br>- Nếu tên option vượt quá giới hạn ký tự → Tự động cắt ngắn và hiển thị "..." ở cuối.<br>- Xóa hết keyword → hiển thị lại placeholder trong ô search của dropdown (CMR-03).<br>- Tap ra ngoài khi keyword trống → tự động chọn lại "Tất cả" (CMR-03).<br>- Nếu không có option nào phù hợp → hiển thị *"Không có dữ liệu."* (CMR-03). |
| 2 | Thủ tục hành chính   | Dropdown (Single-selection) | "Tất cả" | x            | —         | **Quy tắc hiển thị:** (Xem CMR-03)<br>- Placeholder: "Chọn thủ tục hành chính".<br>- Danh sách thủ tục hành chính lấy từ danh mục hệ thống.<br>- Giá trị mặc định: "Tất cả".<br><br>**Quy tắc hành động:** (Xem CMR-02, CMR-03)<br>- Người dùng tap → Mở danh sách lựa chọn → Tap item → Tự động đóng và hiển thị giá trị đã chọn.<br>- Người dùng có thể nhập text để tìm kiếm/filter trong dropdown (tìm kiếm gần đúng).<br>- Option đã chọn được highlight/bold khi mở dropdown list.<br>- Nếu tên option vượt quá giới hạn ký tự → Tự động cắt ngắn và hiển thị "..." ở cuối.<br>- Xóa hết keyword → hiển thị lại placeholder trong ô search của dropdown (CMR-03).<br>- Tap ra ngoài khi keyword trống → tự động chọn lại "Tất cả" (CMR-03).<br>- Nếu không có option nào phù hợp → hiển thị *"Không có dữ liệu."* (CMR-03). |
...
| 4 | Nút "Nhập lại" | Button (Secondary)          | —                         | —           | —         | **Quy tắc hiển thị:**<br>- Nút viền outline màu đỏ, text màu đỏ.<br>- Không đóng Bottom Sheet.<br><br>**Quy tắc hành động:** (Xem CMR-02)<br>- Tap → Reset toàn bộ tiêu chí lọc về giá trị mặc định ("Tất cả", "Tất cả").                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 5 | Icon Điều hướng (>)  | Icon                   | —                    | —           | —         | **Quy tắc hành động:** Icon mũi tên ">" màu xám, nằm ở góc phải Card.<br>- Tap vào bất kỳ đâu trên Card hoặc icon này → Chuyển sang màn hình Chi tiết lịch hẹn. |
...
| # | Tên trường         | Kiểu trường    | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                             |
| - | --------------------- | ----------------- | --------------------- | ------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Tên thủ tục        | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị đầy đủ tên thủ tục. Wrap text nếu dài, không truncate.                                                                   |
| 2 | Lĩnh vực            | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị tên lĩnh vực của thủ tục. Wrap text nếu dài, không truncate.                                                             |
| 3 | Thủ tục hành chính       | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị mức độ thủ tục hành chính (ví dụ: "Dịch vụ công trực tuyến mức độ 4"). Wrap text nếu dài, không truncate.            |
| 4 | Cơ quan thực hiện  | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị tên cơ quan thực hiện thủ tục. Wrap text nếu dài, không truncate.                                                         |

**Section 2 — Thông tin lịch hẹn:**

| #  | Tên trường            | Kiểu trường    | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                                                   |
| -- | ------------------------ | ----------------- | --------------------- | ------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5  | Ngày hẹn nộp          | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị ngày dự kiến nộp hồ sơ. Định dạng: DD/MM/YYYY (Xem CMR-12).<br>- Nếu null → hiển thị "-".                                               |
| 6  | Khung giờ hẹn nộp     | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị khung thời gian tiếp nhận hồ sơ. Định dạng: HH:mm - HH:mm (Xem CMR-12).<br>- Ví dụ: "08:00 - 09:00".<br>- Nếu null → hiển thị "-". |
| 7  | Thời gian đặt        | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị thời điểm người dùng đặt lịch. Định dạng: DD/MM/YYYY HH:mm (Xem CMR-12).<br>- Nếu null → hiển thị "-".                               |
| 8  | Ngày cán bộ hẹn nộp | Label (Read-only) | —                    | —           | —         | **Quy tắc hiển thị:** Hiển thị ngày cán bộ xác nhận/điều chỉnh lịch hẹn. Định dạng: DD/MM/YYYY (Xem CMR-12).<br>- Nếu null → hiển thị "-".                         |
...
3. Hiển thị thông tin chi tiết trong 3 section.
...
- **AC3:** Given danh sách lịch hẹn, When hệ thống hiển thị card, Then thông tin trên Card phải hiển thị đầy đủ 2 icon (Tòa nhà, Người) kèm dữ liệu tương ứng. Nếu bất kỳ field nào null/rỗng → hiển thị "-".
- **AC4:** Given danh sách lịch hẹn, When dữ liệu trả về, Then danh sách sắp xếp theo thời gian đặt giảm dần (mới nhất lên đầu).

**Chi tiết:**

- **AC5:** Given người dùng tap vào card lịch hẹn, When hệ thống trả về dữ liệu, Then màn hình chi tiết hiển thị đầy đủ 3 section thông tin, khớp 100% với dữ liệu từ API. Field "Thời gian đặt" phải nhất quán giữa card và chi tiết.
...
- **AC7:** Given người dùng mở modal bộ lọc, When chọn Lĩnh vực và/hoặc Thủ tục hành chính rồi tap "Áp dụng", Then danh sách được lọc chính xác theo tiêu chí. Hai trường Lĩnh vực và Thủ tục hành chính không phụ thuộc vào nhau.
...
- **AC9:** Given người dùng nhập từ khóa tìm kiếm (tối đa 255 ký tự), When sau debounce 3 giây, Then danh sách hiển thị kết quả tìm kiếm gần đúng trên **toàn bộ các tab** (không chỉ tab hiện tại).
...
| Label field: "Lĩnh vực", "Thủ tục hành chính", "Cơ quan thực hiện"... | Tên lĩnh vực                        |
| Tab: "Tất cả", "Chờ xác nhận", "Đã xác nhận"...         | Tên thủ tục hành chính              |
| Placeholder: "Tìm kiếm thủ tục..."                        | Tên cơ quan                           |
| Nút: "Áp dụng", "Nhập lại"                                |                                         |
| Thông báo lỗi, empty state, toast                         |                                         |
...
| 2026-05-07 | v1 → v2 | Section 2.1 Search — Scope & giới hạn ký tự | Không rõ scope, không giới hạn ký tự | Search toàn bộ các tab, tối đa 255 ký tự (CMR-01) | Làm rõ yêu cầu |
...
| 2026-05-21 | v2.1 → v2.2 | Align CMR Mobile v6.0 | 1. Search box max length 500→255. 2. Bổ sung CMR-03 cho Searchable Dropdown. |
| 2026-05-21 | v2.2 → v2.3 | Dropdown filter | Giá trị mặc định "Tất cả [tên trường]"→"Tất cả". Thêm placeholder "Chọn [tên trường]" cho searchable dropdown. |
| 2026-05-21 | v2.3 → v2.4 | Clean up incorrect CMR references | Thay các mã tracking bằng CMR-xx chuẩn |
| 2026-05-22 | v2.4 → v2.5 | §2.1 Modal row 2 — Rename filter field | "Dịch vụ công", placeholder "Chọn dịch vụ công" | "Thủ tục hành chính", placeholder "Chọn thủ tục hành chính" | Align theo yêu cầu nghiệp vụ |
| 2026-05-22 | v2.4 → v2.5 | §2.1 Card row 5 — Remove field | "Ngày cán bộ hẹn nộp" (icon Lịch) | (Xóa) | Không hiển thị trên card list |
| 2026-05-22 | v2.4 → v2.5 | §2.2 Section 1 — Remove fields | "Mã thủ tục" (row 1), "Đơn vị tiếp nhận" (row 6) | (Xóa) | Loại bỏ field không cần thiết |
| 2026-05-22 | v2.4 → v2.5 | §2.2 Section 1 — Rename field | "Dịch vụ công" | "Thủ tục hành chính" | Align tên trường |
| 2026-05-22 | v2.4 → v2.5 | §2.2 — Remove sections | Section 2 (Thông tin người nộp), Section 4 (Trạng thái), Section 5 (Ghi chú) | (Xóa toàn bộ) | Loại bỏ section không cần thiết |
| 2026-05-22 | v2.4 → v2.5 | §3.2, AC3, AC5, AC7, §4 — Cascade update | Tham chiếu 5 section, 3 icon, "Dịch vụ công" | 3 section, 2 icon, "Thủ tục hành chính" | Đồng bộ sau khi xóa field/section |
