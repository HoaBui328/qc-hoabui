...
**Tiêu đề:** UC95 — Xem danh mục dự án đầu tư địa phương đang có cá nhân, tổ chức quan tâm trên Mobile  
**Ngày tạo:** 14/05/2026  
...
## UC95 — Xem danh mục dự án đầu tư địa phương đang có cá nhân, tổ chức quan tâm

### 1. Mô tả chức năng

- **Tên chức năng:** Xem danh mục dự án đầu tư địa phương đang có cá nhân, tổ chức quan tâm
- **Mô tả:** Chức năng cho phép người dùng xem danh sách các dự án đầu tư thuộc quản lý của các Tỉnh/Thành phố mà đã có cá nhân hoặc tổ chức bày tỏ quan tâm. Đây là danh mục công khai giúp nhà đầu tư đánh giá mức độ cạnh tranh theo địa bàn.
- **Phân quyền:** Toàn bộ người dùng (Khách vãng lai và người dùng đã đăng nhập).
- **Phạm vi ngoài UC (Exclusions):** KHÔNG bao gồm: đăng ký quan tâm dự án, gửi hồ sơ đầu tư, hoặc xem danh mục dự án bộ ngành (UC94).
- **Truy cập chức năng:** Sidebar → **"Xúc tiến đầu tư"** → **"DA địa phương có NĐT quan tâm"**.
...
- **Chức năng đáp ứng usecase số:** UC95 (Phụ lục XIV)

---

### 2. Mô tả giao diện

#### 2.1 Màn hình Danh sách Dự án địa phương có NĐT quan tâm
...
| 2 | Tiêu đề Header | Label | "DA địa phương - NĐT quan tâm" | — | — | Font đậm, màu trắng, giữa Header. |
...
| 5 | Nút Lọc | Button (Icon) | — | — | — | Tap → Bottom Sheet (Tỉnh/TP, Lĩnh vực, Quy mô vốn). Active filter indicator (CMR-02). |
...
| 8 | Tỉnh/Thành phố | Icon + Label | — | — | — | Icon định vị + Tên tỉnh/TP. |
| 9 | Lĩnh vực đầu tư | Icon + Label | — | — | — | Icon ngành + Tên lĩnh vực. |
| 10 | Tổng vốn đầu tư | Icon + Label | — | — | — | Icon $ + Số tiền (màu đỏ, CMR-11). |
| 11 | Số lượt quan tâm | Icon + Label | — | — | — | Icon người + Số lượng (VD: "8 NĐT quan tâm"). Màu xanh dương đậm. |
...
| 14 | Tỉnh/Thành phố | Dropdown | "Tất cả" | x | — | 63 tỉnh/TP. Searchable (CMR-03). |
...
| 5 | Tỉnh/Thành phố | Label | — | — | — | Null → "-" (CMR-14). |
| 6 | Lĩnh vực đầu tư | Label | — | — | — | Null → "-" (CMR-14). |
| 7 | Địa điểm cụ thể | Label | — | — | — | Địa chỉ chi tiết dự án. Null → "-" (CMR-14). |
| 8 | Tổng vốn đầu tư | Label | — | — | — | CMR-11. Null → "-" (CMR-14). |
| 9 | Quy mô dự án | Label (Multiline) | — | — | — | Null → "-" (CMR-14). |
...
| 13 | Ưu đãi đầu tư | Label (Multiline) | — | — | — | Mô tả ưu đãi (nếu có). Null → "-" (CMR-14). |
...
1. API lấy danh sách dự án đầu tư địa phương có NĐT quan tâm.
...
2. **Lọc:** API kèm (ProvinceID, IndustryID, CapitalRange).
...
- **AC1:** Card hiển thị: Tên DA, Tỉnh/TP, Lĩnh vực, Vốn, Số lượt quan tâm.
- **AC2:** Số lượt quan tâm hiển thị nổi bật (màu xanh dương đậm).
- **AC3:** Sắp xếp mặc định theo số lượt quan tâm giảm dần.
- **AC4:** Bộ lọc 3 tiêu chí (Tỉnh/TP, Lĩnh vực, Quy mô vốn).
- **AC5:** Chi tiết hiển thị đầy đủ, null → "-".
- **AC6:** Trường "Ưu đãi đầu tư" hiển thị khi có dữ liệu, giúp NĐT đánh giá lợi ích.
...
|---|---|---|---|---|---|
| 2026-05-14 | v1 | Khởi tạo | (Không có) | Tạo mới SRS UC95 | Assume từ Phụ lục XIV + CMR Mobile |
