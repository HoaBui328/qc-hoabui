...
| # | Tên trường | Kiểu trường | Giá trị mặc định        | Được sửa | Bắt buộc | Mô tả/Ghi chú                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| - | ------------- | -------------- | ---------------------------- | ------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 | Ô tìm kiếm | Text Input     | Placeholder: "Tìm kiếm nhanh theo tên" | —           | —         | **Quy tắc hiển thị:**<br>- Ô tìm kiếm luôn hiển thị bên dưới Header (always visible).<br>- Icon kính lúp nằm bên trái trong ô.<br>- Tìm kiếm gần đúng (chứa từ khóa) tên tài liệu. Khi xóa hết từ khóa, danh sách trở về trạng thái mặc định (hiển thị tất cả).<br>- Max length: **255 ký tự** (Xem CMR-01). Khi nhập đủ 255 ký tự, không cho phép nhập thêm.<br><br>**Quy tắc hành động:** (Xem CMR-01)<br>- Nhập từ khóa → Kết quả hiển thị tự động sau **3 giây debounce**.<br>- Người dùng nhập từ khóa → Kết quả hiển thị ngay, không cần nhấn Enter hay nhấn nút nào.<br>- Tự động trim space (cắt khoảng trắng hai đầu) trước khi tìm kiếm. Nếu sau trim rỗng → không trigger search, danh sách về mặc định.<br>- Nếu không có kết quả trùng khớp: Hiển thị màn *"Không tìm thấy kết quả."* (Xem CMR-14) |
...
| 3 | Item Tập tin     | Interaction    | —                    | —           | —         | <br />**Quy tắc hiển thị:** (Xem CMR-08)<br>- Hiển thị tập tin trong thư mục. Thứ tự: sắp xếp theo tên tài liệu 1-9/A-Z <br>- Mỗi item gồm icon file, tên file, và nút tải file (căn phải).<br>- Tên file dài → truncate bằng "...".<br>- Nếu không có file → hiển thị empty state "Không có dữ liệu." (Xem CMR-14)<br><br>**Quy tắc hành động:** (Xem CMR-08)<br>- **Xem trực tiếp (PDF, JPG, PNG, MP4, AVI, MOV):** Tap vào tên file → Mở xem trực tiếp trên trình duyệt thiết bị.<br>- **Tải xuống (DOC, DOCX, XLS, XLSX, ZIP, v.v.):** Tap vào tên file → Tự động tải xuống máy.<br>- **Định dạng không hỗ trợ:** Hiển thị thông báo *"Định dạng không hỗ trợ. Vui lòng tải xuống."* |
| 4 | Nút Tải file     | Button (Icon + Text)  | —                    | —           | —         | **Quy tắc hiển thị:**<br>- Nút gồm icon download (mũi tên xuống) + chữ "Tải". Căn phải mỗi item tập tin.<br>- Luôn hiển thị cho mọi tập tin.<br><br>**Quy tắc hành động:**<br>- Tap → Tải file xuống thiết bị (download). Áp dụng cho tất cả định dạng file.<br>- Trong khi tải: hiển thị loading indicator trên nút (spinner thay icon download).<br>- Tải thành công: hiển thị toast *"Tải xuống thành công."*<br>- Tải thất bại: hiển thị toast *"Tải xuống thất bại. Vui lòng thử lại."* |
...
1. Ô tìm kiếm luôn hiển thị bên dưới Header (always visible).
2. Người dùng nhập từ khóa vào ô tìm kiếm.
3. Hệ thống thực hiện lọc real-time:
...
| 2026-05-11 | v1.1 → v2  | Section 2.1 & 2.2 — Ô tìm kiếm (Max length)      | (Không có)                                                                                                                  | Max length: 255 ký tự (Xem CMR-01). Khi nhập đủ 255 ký tự, không cho phép nhập thêm.                                                                                                                                            | Bổ sung max length chuẩn CMR-01 cho cả 2 màn tìm kiếm (khắc phục inconsistency Nhóm F)                                   |
...
| 2026-05-13 | v4 → v5    | Section 2.1 — Khung Header (Row #3 Nút Tìm kiếm) | Có row Nút Tìm kiếm (Button Icon): Tap → Hiển thị ô tìm kiếm | (Đã xóa) — Bỏ nút Tìm kiếm trên Header | Ô tìm kiếm chuyển sang always visible, không cần nút trigger |
| 2026-05-13 | v4 → v5    | Section 2.1 — Ô tìm kiếm (Tiêu đề + Quy tắc) | "Ô tìm kiếm (Khi tap nút Tìm kiếm):" / Không có rule always visible | "Ô tìm kiếm:" / Thêm rule: Ô tìm kiếm luôn hiển thị bên dưới Header (always visible) | Ô tìm kiếm luôn hiển thị, không cần tap nút |
| 2026-05-13 | v4 → v5    | Section 2.2 — Ô tìm kiếm (Tiêu đề + Quy tắc) | "Ô tìm kiếm (Khi tap nút Tìm kiếm):" / Không có rule always visible | "Ô tìm kiếm:" / Thêm rule: Ô tìm kiếm luôn hiển thị bên dưới Header (always visible) | Đồng bộ với màn 2.1 |
| 2026-05-13 | v4 → v5    | Section 3.1 — Luồng xử lý Tìm kiếm (Bước 1-2) | Bước 1: Người dùng nhấn icon Tìm kiếm. Bước 2: Hệ thống hiển thị ô tìm kiếm. | Bước 1: Ô tìm kiếm luôn hiển thị bên dưới Header. Bước 2: Người dùng nhập từ khóa vào ô tìm kiếm. | Bỏ bước nhấn icon, gộp lại do ô tìm kiếm always visible |
| 2026-05-21 | v5 → v5.1  | Section 2.1 — Card Thư mục (hiển thị thư mục rỗng) | (Không mô tả rõ) | Thêm: "Hiển thị tất cả thư mục kể cả thư mục rỗng (0 tài liệu). Không ẩn thư mục rỗng." | BA feedback — làm rõ hiển thị thư mục rỗng |
| 2026-05-21 | v5 → v5.1  | Section 2.2 — Danh sách Tập tin (nút Tải file) | Mỗi item gồm icon file, tên file. Không có nút tải riêng | Thêm row #4: Nút Tải file (Button Icon + Text "Tải") căn phải mỗi item; tap → download; loading spinner khi tải; toast thành công/thất bại | BA feedback — đồng bộ với UC53 Phản ánh kiến nghị |
| 2026-05-21 | v5.1 → v5.2 | Section 2.1 & 2.2 — Ô tìm kiếm (Placeholder) | "tìm kiếm tên" | "Tìm kiếm nhanh theo tên" | BA feedback — đổi placeholder |
| 2026-05-21 | v5.2 → v5.3 | Align CMR Mobile v6.0 | Search box max length 500→255. |
| 2026-05-21 | v5.3 → v5.4 | Section 2.1 & 2.2 — Ô tìm kiếm (Auto-trim) | (Không có) | Bổ sung: Tự động trim space (cắt khoảng trắng hai đầu) trước khi tìm kiếm. Nếu sau trim rỗng → không trigger search, danh sách về mặc định. | Đồng bộ CMR-01 |
