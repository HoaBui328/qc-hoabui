...
- **Mô tả:** Chức năng cho phép cá nhân, tổ chức tra cứu, xem danh sách và xem chi tiết các thủ tục hành chính. Người dùng có thể tìm kiếm theo mã, tên dịch vụ công; sử dụng bộ lọc (lĩnh vực); và xem chi tiết thủ tục được tổ chức theo 5 tab nội dung: Thông tin chung, Trình tự thực hiện, Thành phần hồ sơ, Cách thức thực hiện, Căn cứ pháp lý.
...
| 4 | Ô tìm kiếm | Textbox (Search) | "Tìm kiếm nhanh theo mã, tên dịch vụ công" | x | — | **Quy tắc hiển thị:**<br>- Nền trắng, viền xám mỏng. Placeholder: "Tìm kiếm nhanh theo mã, tên dịch vụ công".<br>- Tối đa **255 ký tự** (Xem CMR-01).<br><br>**Quy tắc hành động:**<br>- Nhập từ khóa → Hệ thống tìm kiếm theo tên hoặc mã thủ tục. Kết quả tự động cập nhật sau debounce 3 giây (Tham khảo CMR-01).<br>- Xử lý whitespace: auto-trim khoảng trắng đầu/cuối trước khi tìm kiếm. Nếu sau trim rỗng → coi như input rỗng (Xem CMR-01).<br>- Xóa hết từ khóa → Hiển thị danh sách mặc định.<br>- Khi chuyển sang màn hình khác → Reset search/filter về trạng thái mặc định (CMR-01).<br>- State Persistence: Sau khi vào chi tiết và quay lại, danh sách giữ nguyên trạng thái tìm kiếm/lọc trước đó (CMR-01). |
| 5 | Nút "Lọc" | Button (Icon) | — | — | — | **Quy tắc hiển thị:**<br>- Icon filter, nằm bên phải ô tìm kiếm.<br>- **Active filter indicator:** Khi đang có bộ lọc được áp dụng (Lĩnh vực khác mặc định), hiển thị chấm xanh (indicator) trên icon báo hiệu trạng thái đang lọc (Xem CMR-02).<br><br>**Quy tắc hành động:**<br>- Tap → Mở Modal popup "Bộ lọc tìm kiếm". |
...
| 8 | Các thuộc tính thẻ | Label / Value | — | — | — | **Quy tắc hiển thị:**<br>Hiển thị danh sách các thuộc tính theo cặp nhãn - giá trị:<br>- **Mã thủ tục:** (VD: TT-001-2024)<br>- **Lĩnh vực:** (VD: Đầu tư)<br>- **Cơ quan thực hiện:** (VD: Bộ Kế hoạch và Đầu tư)<br>Tất cả text nếu dài quá sẽ hiển thị `...`. Nếu null hiển thị "-". |
...
| 2 | Lĩnh vực | Dropdown (Single) | "Tất cả lĩnh vực" | x | — | **Quy tắc hiển thị:**<br>- List options trả về từ API. Mặc định "Tất cả lĩnh vực".<br><br>**Quy tắc hành động:**<br>- Áp dụng D07: Xóa hết keyword tìm kiếm trong dropdown → hiển thị lại placeholder "Tất cả lĩnh vực".<br>- Áp dụng D08: Tap ra ngoài khi keyword đang trống → tự động chọn lại "Tất cả lĩnh vực".<br>- Khi nhập keyword không có option trùng khớp → hiển thị "Không có dữ liệu" (CMR-03). |
| 3 | Nút "Nhập lại" | Button (Outline) | — | — | — | **Quy tắc hiển thị:**<br>- Nằm bên trái, viền nhạt, chữ đen, có icon refresh. **Luôn Enabled** (I07).<br><br>**Quy tắc hành động:**<br>- Tap → Reset tất cả field về giá trị mặc định, giữ nguyên Bottom Sheet. |
| 4 | Nút "Áp dụng" | Button (Primary) | — | — | — | **Quy tắc hiển thị:**<br>- Nằm bên phải, nền đỏ đậm, chữ trắng. **Luôn Enabled** (CMR-09, I06).<br><br>**Quy tắc hành động:**<br>- Tap → Đóng Bottom Sheet, tải lại danh sách kết quả với điều kiện lọc đã chọn.<br>- Khi đang gọi API → hiển thị loading state, disable tạm thời cho đến khi API trả về (I02). |
...
| 5 | **Tab Thông tin chung** - Khối Tổng quan | List Attribute | — | — | — | **Quy tắc hiển thị:** Hiển thị 2 trường cốt lõi:<br>- Cơ quan thực hiện (VD: Bộ Kế hoạch và Đầu tư)<br>- Thời hạn (VD: 15 ngày làm việc) |
...
4. Khi nhấn nút Lọc, Bottom Sheet mở lên. Bấm "Áp dụng" để áp dụng bộ lọc Lĩnh vực cùng với ô tìm kiếm. Tap ra ngoài hoặc tap Đóng để thoát không áp dụng.
...
- **AC2:** Bottom Sheet bộ lọc (Lĩnh vực) kết hợp với ô tìm kiếm khi bấm "Áp dụng". Nút "Nhập lại" reset về mặc định. Icon Lọc hiển thị chấm xanh (indicator) khi đang lọc.
...
| 18/05/2026 | v5.0 | Chỉnh sửa nhãn & bỏ bộ lọc Ngôn ngữ | "Cơ quan" trên card & Tổng quan; Bộ lọc có Ngôn ngữ | Đổi "Cơ quan" → "Cơ quan thực hiện"; Xóa bộ lọc Ngôn ngữ khỏi Bottom Sheet và các mô tả liên quan. | Theo yêu cầu BA ngày 18/05 |
| 2026-05-21 | v5.0 → v5.1 | Align CMR Mobile v6.0 | Search box max length 500→255. |
| 2026-05-21 | v5.1 → v5.2 | Align CMR Mobile v6.0 | 1. Search box max length 500→255 |
| 2026-05-21 | v5.2 → v5.3 | Clean up incorrect CMR references | Thay các mã tracking bằng CMR-xx chuẩn |
| 2026-05-21 | v5.3 → v5.4 | Nút "Áp dụng" filter | (Chưa tường minh trạng thái) | Bổ sung: **Luôn Enabled** (CMR-09). Không có trạng thái Disabled. | Đồng bộ CMR-09 |
| 2026-05-21 | v5.3 → v5.4 | Align CMR v6.0 — Bỏ `.` error messages | `"Không tìm thấy kết quả."`, `"Không thể kết nối...thử lại."`, `"Nội dung không tồn tại...xóa."` | Bỏ dấu `.` cuối tất cả error/empty state messages | Align theo rule H — CMR Align v6.0 |
| 2026-05-21 | v5.3 → v5.4 | Align CMR v6.0 — Dropdown Lĩnh vực | Chỉ ghi "List options trả về từ API" | Bổ sung D07 (clear keyword → placeholder) và D08 (tap ngoài khi trống → "Tất cả lĩnh vực") | Align theo rule D07/D08 — CMR Align v6.0 |
| 2026-05-21 | v5.3 → v5.4 | Align CMR v6.0 — Nút Áp dụng / Nhập lại | Không ghi trạng thái enabled/disabled | Bổ sung: Luôn Enabled (I06, I07). Loading state khi gọi API (I02) | Align theo rule I06/I07 — CMR Align v6.0 |
| 2026-05-22 | v5.4 → v5.5 | §2.1 row 4 — Reset khi chuyển màn | Không mô tả reset search/filter | Bổ sung: Khi chuyển sang màn hình khác → Reset search/filter về trạng thái mặc định (CMR-01) | Align CMR-01 rule "Reset khi chuyển màn" |
| 2026-05-22 | v5.4 → v5.5 | §2.2 row 2 — Dropdown empty state | Không mô tả trường hợp không có option | Bổ sung: Khi nhập keyword không có option trùng khớp → hiển thị "Không có dữ liệu" (CMR-03) | Align CMR-03 |
