...
- Trường bắt buộc "Địa bàn mong muốn": Hiển thị placeholder "Chọn..." (chưa chọn giá trị).
- Các Dropdown không bắt buộc (Ngành kinh tế, Hình thức đầu tư, Quốc gia hợp tác): Hiển thị placeholder "Chọn..." (chưa chọn giá trị).
- Textbox (Quy mô vốn): Để trống, hiển thị placeholder.
- Textarea (Mô tả nhu cầu đầu tư): Để trống, hiển thị placeholder.
- Nút "Xem dự án phù hợp": Disabled (do chưa điền đủ trường bắt buộc).
...
- **No data (không có dữ liệu):** Khi hệ thống chưa có dự án kêu gọi đầu tư nào (API trả về danh sách rỗng ngay từ đầu, không qua filter/search) → Hiển thị hình ảnh trống + thông báo: *"Không có dữ liệu."* (Tham chiếu: UC87 - Dự án đề xuất - Empty state.png, CMR-14).
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 3 | Ô tìm kiếm | Textbox (Search) | — | x | — | **Quy tắc hiển thị:**<br>- Placeholder: "Tìm kiếm nhanh theo tên dự án". Icon kính lúp bên trái. Tối đa 500 ký tự (CMR-01).<br>**Quy tắc hành động:**<br>- Debounce 3 giây (CMR-01). Tìm theo Tên dự án.<br>- Auto-trim whitespace (CMR-01). |
...
| 9 | Tổng vốn đầu tư dự kiến | Icon + Label | — | — | — | **Quy tắc hiển thị:**<br>- Icon tiền (₫) + tiền tố "Vốn: " + Số tiền (màu đỏ) + đơn vị "tỷ VNĐ". VD: `⊙ Vốn: 8.200 tỷ VNĐ`. Tự động phân tách hàng nghìn bằng dấu phẩy, thập phân bằng dấu chấm (CMR-11). |
...
| 13 | Loại dự án | Dropdown | "Tất cả" | x | — | Giá trị mặc định: "Tất cả". Tất cả / Trong nước / Nước ngoài / Trong & ngoài nước. |
| 14 | Lĩnh vực | Dropdown | "Tất cả" | x | — | Giá trị mặc định: "Tất cả". Searchable (CMR-03). |
| 15 | Ngành kinh tế | Dropdown | "Tất cả" | x | — | Giá trị mặc định: "Tất cả". Searchable (CMR-03). |
| 16 | Địa bàn (Tỉnh/TP) | Dropdown | "Tất cả" | x | — | Giá trị mặc định: "Tất cả". 63 tỉnh/TP. Searchable (CMR-03). |
| 17 | Hình thức đầu tư | Dropdown | "Tất cả" | x | — | Giá trị mặc định: "Tất cả". Tất cả / 100% vốn nước ngoài / Liên doanh / Hợp đồng BCC / Khác. |
...
| 7 | Địa bàn | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Hiển thị đầy đủ, tự động xuống dòng (có thể bao gồm Huyện + Tỉnh/TP). Null → `-` (CMR-14). |
| 8 | Hình thức đầu tư | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 9 | Tiêu đề section | Label | "2. Cơ quan quản lý" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 10 | Cơ quan chủ quản | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Hiển thị đầy đủ, tự động xuống dòng. Null → `-` (CMR-14). |
| 11 | Đơn vị triển khai | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Hiển thị đầy đủ, tự động xuống dòng. Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 12 | Tiêu đề section | Label | "3. Thông tin liên hệ" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 13 | Người liên hệ | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Null → `-` (CMR-14). |
| 14 | Chức vụ | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font thường. Null → `-` (CMR-14). |
| 15 | Email | Label | — | — | — | **Quy tắc hiển thị:**<br>- Read-only, hiển thị dạng text thuần (không phải link tappable). Null → `-` (CMR-14). |
| 16 | Điện thoại | Label | — | — | — | **Quy tắc hiển thị:**<br>- Read-only, hiển thị dạng text thuần (không phải link tappable). Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 17 | Tiêu đề section | Label | "4. Mục tiêu, quy mô dự án" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 18 | Mục tiêu dự án | Label (Multiline) | — | — | — | **Quy tắc hiển thị:**<br>- Hiển thị đầy đủ, tự động xuống dòng. Null → `-` (CMR-14). |
| 19 | Quy mô diện tích | Label | — | — | — | **Quy tắc hiển thị:**<br>- Đơn vị: hecta hoặc m². Null → `-` (CMR-14). |
| 20 | Công suất | Label (Multiline) | — | — | — | **Quy tắc hiển thị:**<br>- Hiển thị đầy đủ, tự động xuống dòng. Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 21 | Tiêu đề section | Label | "5. Nguồn vốn dự án" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 22 | Tổng vốn đầu tư | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ, cỡ lớn. Đơn vị: tỷ VNĐ. VD: `12.500 tỷ VNĐ`. Tự động phân tách hàng nghìn bằng dấu phẩy, thập phân bằng dấu chấm (CMR-11). Null → `-` (CMR-14). |
| 23 | Vốn nhà nước | Label | — | — | — | **Quy tắc hiển thị:**<br>- Hiển thị trong khung bo góc. Số tiền kèm tỷ lệ phần trăm (%). VD: "4.000 tỷ VND (32%)". CMR-11. Null → `-` (CMR-14). |
| 24 | Vốn tư nhân/FDI | Label | — | — | — | **Quy tắc hiển thị:**<br>- Hiển thị trong khung bo góc (bên cạnh Vốn nhà nước). Số tiền kèm tỷ lệ phần trăm (%). VD: "8.500 tỷ VND (68%)". CMR-11. Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 25 | Tiêu đề section | Label | "6. Tiến độ dự án" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 26 | Thời gian bắt đầu | Label | — | — | — | **Quy tắc hiển thị:**<br>- Format: Quý/Năm (VD: "Quý 2/2026"). Null → `-` (CMR-14). |
| 27 | Thời gian hoàn thành | Label | — | — | — | **Quy tắc hiển thị:**<br>- Format: Quý/Năm (VD: "Quý 4/2029"). Null → `-` (CMR-14). |
| 28 | Tình trạng hiện tại | Badge + Label | — | — | — | **Quy tắc hiển thị:**<br>- Hiển thị badge trạng thái (icon + nền màu) + mô tả chi tiết. Hiển thị đầy đủ, tự động xuống dòng. Null → `-` (CMR-14). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 29 | Tiêu đề section | Label | "7. Tài liệu đính kèm" | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ đậm, căn trái. |
| 30 | Danh sách tài liệu | List (File Item) | — | — | — | **Quy tắc hiển thị:**<br>- Mỗi item: Icon loại file + Tên tài liệu + Nút "Tải xuống" (icon download).<br>- Tap nút "Tải xuống" → Xem/Tải theo CMR-08.<br>- **Trạng thái Empty:** Nếu không có tài liệu → Ẩn toàn bộ section. |
...
| 2026-05-21 | v2.0 → v2.1 | §2.2 — Trạng thái mặc định form | (Không có) | Bổ sung mô tả trạng thái mặc định khi NĐT mở form lần đầu: Lĩnh vực "Tất cả", các dropdown khác "Chọn...", textbox/textarea trống — theo wireframe | QC feedback |
