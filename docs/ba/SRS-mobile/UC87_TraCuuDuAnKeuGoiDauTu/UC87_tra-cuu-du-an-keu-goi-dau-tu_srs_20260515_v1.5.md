...
- **Phân quyền:** Người dùng đã đăng nhập (Cá nhân / Tổ chức). Khách vãng lai KHÔNG truy cập được. Cá nhân và Tổ chức có cùng giao diện và chức năng, không phân biệt.
...
- **Điều kiện kết thúc (Postconditions):** Hệ thống hiển thị danh sách dự án phù hợp hoặc cho phép NĐT đăng ký quan tâm dự án.
...
| 3 | Icon Thông báo (Chuông) | Button (Icon) | — | — | — | **Quy tắc hiển thị:**<br>- Icon chuông trắng + Badge dạng dot đỏ (không hiển thị số) khi có thông báo mới. Không có thông báo mới → ẩn dot.<br>**Quy tắc hành động:**<br>- Tap → Điều hướng đến Trung tâm thông báo (UC riêng).<br>- Debounce Navigation (CMR-18). |
| 4 | Card "Bổ sung thông tin NĐT" | Card (Navigation) | — | — | — | **Quy tắc hiển thị:**<br>- Nền trắng, bo góc, shadow nhẹ. Icon minh họa + Tiêu đề "Bổ sung thông tin nhà đầu tư - Gợi ý dự án phù hợp" + Mô tả ngắn.<br>**Quy tắc hành động:**<br>- Tap → Điều hướng đến Màn hình 2.2 (Bổ sung thông tin NĐT).<br>- Debounce Navigation (CMR-18). |
| 5 | Card "Danh sách dự án đề xuất" | Card (Navigation) | — | — | — | **Quy tắc hiển thị:**<br>- Nền trắng, bo góc, shadow nhẹ. Icon danh sách + Tiêu đề "Danh sách dự án đề xuất cho nhà đầu tư" + Mô tả ngắn.<br>**Quy tắc hành động:**<br>- Tap → Điều hướng đến Màn hình 2.3 (Danh sách dự án đề xuất).<br>- Debounce Navigation (CMR-18). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 1 | Nút Quay lại (←) | Button (Icon) | — | — | — | **Quy tắc hành động:**<br>- Tap → Nếu form có thay đổi chưa lưu (dirty state) → Hiển thị Confirmation Dialog (CMR-10): "Thông tin chưa được lưu. Bạn có muốn rời đi?" + "Ở lại" (Secondary) / "Rời đi" (Primary). Tap "Rời đi" → Quay về Hub (2.1). Tap "Ở lại" → Đóng dialog, giữ nguyên form.<br>- Nếu form không thay đổi → Quay về Hub (2.1) ngay. (CMR-06) |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 3 | Lĩnh vực quan tâm | Dropdown (Multi-select) | — | x | x | **Quy tắc hiển thị:**<br>- Placeholder: "Chọn lĩnh vực quan tâm...".<br>- Cho phép chọn nhiều lĩnh vực, không giới hạn số lượng. Danh sách từ API danh mục.<br>- Hiển thị dạng chip/tag, wrap xuống dòng khi hết chiều rộng. Mỗi chip có nút X để xóa.<br>**Quy tắc hành động:**<br>- Inline error nếu bỏ trống: "Vui lòng chọn [tên trường]" (CMR-09) |
| 4 | Ngành kinh tế | Dropdown (Multi-select) | — | x | — | **Quy tắc hiển thị:**<br>- Placeholder: "Chọn ngành kinh tế...". Searchable (CMR-03).<br>- Không giới hạn số lượng. Hiển thị dạng chip/tag. |
| 5 | Địa bàn mong muốn | Dropdown (Multi-select) | — | x | x | **Quy tắc hiển thị:**<br>- Placeholder: "Chọn tỉnh/thành phố...". Danh sách 63 tỉnh/TP. Searchable (CMR-03).<br>- Không giới hạn số lượng. Hiển thị dạng chip/tag.<br>- Inline error nếu bỏ trống (CMR-09). |
| 6 | Hình thức đầu tư | Dropdown | — | x | — | **Quy tắc hiển thị:**<br>- Giá trị: 100% vốn nước ngoài / Liên doanh / Hợp đồng BCC / Khác. |
| 7 | Quy mô vốn dự kiến | Textbox (Number) | — | x | — | **Quy tắc hiển thị:**<br>- Chỉ nhập số (CMR-11). Đơn vị: VNĐ (cố định, hiển thị label bên phải textbox).<br>- Tối đa 500 ký tự (CMR-11). |
| 8 | Quốc gia mong muốn hợp tác | Dropdown (Multi-select) | — | x | — | **Quy tắc hiển thị:**<br>- Danh sách quốc gia từ API. Searchable (CMR-03).<br>- Không giới hạn số lượng. Hiển thị dạng chip/tag. |
| 9 | Mô tả nhu cầu đầu tư | Textarea | — | x | — | **Quy tắc hiển thị:**<br>- Placeholder: "Mô tả chi tiết nhu cầu đầu tư của bạn...".<br>- Tối đa 2000 ký tự. Auto-trim whitespace (CMR-09). |
...
#### 2.3 Màn hình Danh sách dự án đề xuất cho nhà đầu tư (Tham chiếu: UC87 - Dự án đề xuất.png, UC87 - Chưa có gợi ý.png)
...
Nếu NĐT chưa từng bổ sung thông tin nhu cầu đầu tư (chưa thực hiện Chức năng 1), màn hình hiển thị danh sách rỗng kèm thông báo: *"Vui lòng bổ sung thông tin để nhận gợi ý dự án phù hợp."* và nút **"Bổ sung ngay"** (tap → điều hướng đến Màn hình 2.2).
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 3 | Ô tìm kiếm | Textbox (Search) | "Tìm kiếm dự án..." | x | — | **Quy tắc hiển thị:**<br>- Icon kính lúp bên trái. Tối đa 500 ký tự (CMR-01).<br>**Quy tắc hành động:**<br>- Debounce 3 giây (CMR-01). Tìm theo Tên dự án, Mã dự án.<br>- Auto-trim whitespace (CMR-01). |
...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 5 | Thẻ dự án (Card) | Container | — | — | — | **Quy tắc hiển thị:**<br>- Khung trắng, bo góc, shadow nhẹ. Lazy load 20 bản ghi/lần (CMR-04).<br>- Hỗ trợ Pull to Refresh (CMR-13). |
| 6 | Mã dự án | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font nhỏ, màu xám. Nằm góc trên phải Card. |
| 7 | Tên dự án | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font đậm (H2), màu đen. Tối đa 2 dòng, truncate `...`. |
| 8 | Loại dự án | Badge | — | — | — | **Quy tắc hiển thị:**<br>- Badge nhỏ: "Trong nước" (xanh dương) / "Nước ngoài" (cam) / "Trong & ngoài nước" (tím). |
| 9 | Lĩnh vực | Icon + Label | — | — | — | **Quy tắc hiển thị:**<br>- Icon ngành + Tên lĩnh vực. |
| 10 | Địa bàn | Icon + Label | — | — | — | **Quy tắc hiển thị:**<br>- Icon định vị + Tên tỉnh/TP hoặc KCN. |
| 11 | Tổng vốn đầu tư dự kiến | Icon + Label | — | — | — | **Quy tắc hiển thị:**<br>- Icon $ + Số tiền (màu đỏ). Định dạng CMR-11. |
| 12 | Hình thức đầu tư | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font nhỏ, màu xám. VD: "100% vốn nước ngoài". |
| 13 | Nút "Đăng ký quan tâm" / "Hủy quan tâm" | Button (Primary/Secondary, Small) | — | — | — | **Quy tắc hiển thị:**<br>- Trạng thái mặc định: "Đăng ký quan tâm" (nền đỏ, text trắng).<br>- Sau khi đăng ký thành công: Đổi thành "Hủy quan tâm" (nền trắng, viền xám, text xám).<br>**Quy tắc hành động:**<br>- Tap "Đăng ký quan tâm" → API đăng ký → Toast "Đăng ký quan tâm thành công." → Đổi trạng thái nút.<br>- Tap "Hủy quan tâm" → API hủy → Toast "Đã hủy đăng ký quan tâm." → Đổi lại trạng thái nút.<br>- Không giới hạn số dự án đăng ký quan tâm.<br>- Debounce (CMR-18). |
| 14 | Nút "Chi tiết" | Button (Secondary, Small) | — | — | — | **Quy tắc hành động:**<br>- Tap → Điều hướng đến Màn hình 2.4 (Chi tiết dự án).<br>- Debounce Navigation (CMR-18). |
...
| 16 | Loại dự án | Dropdown | "Tất cả" | x | — | Tất cả / Trong nước / Nước ngoài / Trong & ngoài nước. |
| 17 | Lĩnh vực | Dropdown | "Tất cả" | x | — | Searchable (CMR-03). |
| 18 | Ngành kinh tế | Dropdown | "Tất cả" | x | — | Searchable (CMR-03). |
| 19 | Địa bàn (Tỉnh/TP) | Dropdown | "Tất cả" | x | — | 63 tỉnh/TP. Searchable (CMR-03). |
| 20 | Hình thức đầu tư | Dropdown | "Tất cả" | x | — | 100% vốn nước ngoài / Liên doanh / Hợp đồng BCC / Khác. |
...
| 26 | Quy mô vốn thu hút đầu tư | Label | — | — | — | **Quy tắc hiển thị:**<br>- Định dạng CMR-11. Null → `-` (CMR-14). |
...
| 34 | Tổng vốn đầu tư dự kiến | Label | — | — | — | **Quy tắc hiển thị:**<br>- Font đậm, màu đỏ. Định dạng CMR-11. Null → `-` (CMR-14). |
| 35 | Ngân sách nhà nước | Label | — | — | — | CMR-11. Null → `-` (CMR-14). |
| 36 | Vốn nhà đầu tư | Label | — | — | — | CMR-11. Null → `-` (CMR-14). |
| 37 | Vốn vay | Label | — | — | — | CMR-11. Null → `-` (CMR-14). |
| 38 | Khác (tài trợ) | Label | — | — | — | CMR-11. Null → `-` (CMR-14). |
| 39 | Bảng hạng mục chi phí | Table (Scrollable) | — | — | — | **Quy tắc hiển thị:**<br>- Bảng cuộn ngang nếu vượt chiều rộng màn hình.<br>- Cột: Hạng mục chi phí đầu tư \| Nhóm chi phí \| NSNN \| Vốn vay \| Khác \| Tổng cộng.<br>- Định dạng số CMR-11.<br>- **Trạng thái Empty:** Nếu không có dữ liệu → "Không có dữ liệu." (CMR-14). |
...
7. Nếu không có dự án phù hợp → Hiển thị "Không tìm thấy dữ liệu" (CMR-14).
...
3. Thành công → Toast "Đăng ký quan tâm thành công." → Nút đổi thành "Hủy quan tâm".
4. NĐT tap "Hủy quan tâm" → Hệ thống gọi API hủy → Toast "Đã hủy đăng ký quan tâm." → Nút đổi lại "Đăng ký quan tâm".
5. Không giới hạn số dự án đăng ký quan tâm.
6. **Data sync:** Khi NĐT đăng ký/hủy quan tâm ở màn Chi tiết (§2.4) rồi quay lại Danh sách (§2.3), trạng thái nút trên Card tương ứng được cập nhật ngay (local state sync, không cần pull-to-refresh).
7. Thất bại → Hiển thị lỗi theo CMR-07.
...
| 2026-05-15 | v1.4 → v1.5 | §2.2 Back dirty form | (Không có) | Bổ sung: Confirmation Dialog (CMR-10) khi form dirty + nhấn Back | Audit Q10 |
