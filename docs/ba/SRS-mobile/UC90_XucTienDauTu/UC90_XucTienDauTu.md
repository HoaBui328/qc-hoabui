...
| # | Tên trường | Kiểu trường | Giá trị mặc định | Được sửa | Bắt buộc | Mô tả/Ghi chú |
|---|---|---|---|---|---|---|
| 4 | Ô tìm kiếm | Textbox (Search) | "Tìm kiếm nhanh theo tên dự án" | x | — | **Quy tắc hiển thị:** Nền xám nhạt, có icon kính lúp bên trái. Tối đa **255 ký tự** (Xem CMR-01).<br>**Quy tắc hành động:** Nhập từ khóa → Hệ thống tìm kiếm theo Tên dự án và Mô tả ngắn. Tự động lọc sau khi người dùng ngừng gõ 3 giây (Debounce). (Xem CMR-01).<br>- Xử lý whitespace: auto-trim khoảng trắng đầu/cuối trước khi tìm kiếm. Nếu sau trim rỗng → coi như input rỗng (Xem CMR-01). |
...
| 2026-05-11 | v1.1 → v1.2 | Ô tìm kiếm | Không có max length, không có whitespace | Bổ sung: tối đa 255 ký tự (CMR-01), auto-trim whitespace đầu/cuối trước khi tìm kiếm | Đồng bộ CMR-01 (C3) |
| 2026-05-21 | v1.2 → v1.3 | Align CMR Mobile v6.0 | Search box max length 500→255. |
| 2026-05-21 | v1.3 → v1.4 | Clean up incorrect CMR references | Thay các mã tracking bằng CMR-xx chuẩn |
